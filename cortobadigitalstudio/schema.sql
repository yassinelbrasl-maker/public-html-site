-- ============================================================
--  CORTOBA DIGITAL STUDIO — Schéma de base de données
--  Copie commerciale indépendante (préfixe CDS_)
--  Exécuter dans phpMyAdmin (cPanel) — même base que Cortoba Atelier
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `CDS_accounts` (
...
-- Clients
CREATE TABLE IF NOT EXISTS `CDS_clients` (
  `id` varchar(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_clients_contacts_aux` (
...
-- Projets
CREATE TABLE IF NOT EXISTS `CDS_projets` (
  `id` varchar(32) NOT NULL,
...
-- Missions par projet
CREATE TABLE IF NOT EXISTS `CDS_projets_missions` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `CDS_projets_intervenants` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `projet_id` varchar(32) NOT NULL,
...
-- Devis
CREATE TABLE IF NOT EXISTS `CDS_devis` (
  `id` varchar(32) NOT NULL,
...
-- Factures
CREATE TABLE IF NOT EXISTS `CDS_factures` (
  `id` varchar(32) NOT NULL,
  `numero` varchar(40) DEFAULT NULL,
  `client` varchar(200) DEFAULT NULL,
  `client_id` varchar(32) DEFAULT NULL,
...
-- Dépenses
CREATE TABLE IF NOT EXISTS `CDS_depenses` (
  `id` varchar(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_parametres` (
...
CREATE TABLE IF NOT EXISTS `CDS_demandes` (
  `id` varchar(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_taches` (
  `id` varchar(32) NOT NULL,
  `projet_id` varchar(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_journal` (
  `id` varchar(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_demandes_admin` (
  `id` varchar(32) NOT NULL,
  `projet_id` varchar(32) DEFAULT NULL,
  `client_id` varchar(32) DEFAULT NULL,
...
-- ALTER TABLE `CDS_demandes_admin` ADD COLUMN IF NOT EXISTS `client_id` varchar(32) DEFAULT NULL AFTER `projet_id`;
-- ALTER TABLE `CDS_demandes_admin` ADD KEY IF NOT EXISTS `client_id` (`client_id`);

SET FOREIGN_KEY_CHECKS = 1;

-- ════════════════════════════════════════════════════════════
-- MIGRATION : ajouter les colonnes configurateur à CDS_projets
-- Exécuter si la table existe déjà sans ces colonnes
-- ════════════════════════════════════════════════════════════
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `surface_shon` decimal(12,2) DEFAULT NULL;
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `surface_shob` decimal(12,2) DEFAULT NULL;
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `surface_terrain` decimal(12,2) DEFAULT NULL;
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `standing` varchar(40) DEFAULT NULL;
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `zone` varchar(40) DEFAULT NULL;
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `cout_construction` decimal(14,2) DEFAULT NULL;
-- ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `cout_m2` decimal(10,2) DEFAULT NULL;

-- ════════════════════════════════════════════════════════════
-- MIGRATION : créer la table CDS_demandes (configurateur public)
-- Exécuter si la table n'existe pas encore
-- ════════════════════════════════════════════════════════════
-- CREATE TABLE IF NOT EXISTS `CDS_demandes` ( ... );  -- voir définition complète ci-dessus

-- ════════════════════════════════════════════════════════════
-- MIGRATION : ajouter les colonnes de liaison à CDS_demandes
-- Exécuter si la table existe déjà sans ces colonnes
-- ════════════════════════════════════════════════════════════
-- ALTER TABLE `CDS_demandes` ADD COLUMN IF NOT EXISTS `client_id` varchar(32) DEFAULT NULL;
-- ALTER TABLE `CDS_demandes` ADD COLUMN IF NOT EXISTS `projet_id` varchar(32) DEFAULT NULL;
-- ALTER TABLE `CDS_demandes` ADD COLUMN IF NOT EXISTS `devis_id` varchar(32) DEFAULT NULL;
-- ALTER TABLE `CDS_demandes` ADD COLUMN IF NOT EXISTS `traite_par` varchar(120) DEFAULT NULL;
-- ALTER TABLE `CDS_demandes` ADD COLUMN IF NOT EXISTS `traite_at` datetime DEFAULT NULL;
...
-- ════════════════════════════════════════════════════════════
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `profile_picture_url` VARCHAR(400) DEFAULT NULL;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `tel_pro`             VARCHAR(50)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `tel_perso`           VARCHAR(50)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `tel_principal`       ENUM('pro','perso') NOT NULL DEFAULT 'pro';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `email_pro`           VARCHAR(191) DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `email_perso`         VARCHAR(191) DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `email_principal`     ENUM('pro','perso') NOT NULL DEFAULT 'pro';
-- Rémunération (sensible — uniquement retourné aux gérants)
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `salaire_net`         DECIMAL(12,2) DEFAULT 0;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `charges_sociales`    DECIMAL(12,2) DEFAULT 0;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `subventions`         DECIMAL(12,2) DEFAULT 0;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `subv_directe`        TINYINT(1)    DEFAULT 0;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `avantages_nature`    DECIMAL(12,2) DEFAULT 0;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `heures_mois`         DECIMAL(6,2)  DEFAULT 160;
-- Projection d'augmentation
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `date_embauche`       DATE         DEFAULT NULL;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `date_derniere_augm`  DATE         DEFAULT NULL;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `taux_augm_pct`       DECIMAL(5,2) DEFAULT 5;

-- ════════════════════════════════════════════════════════════
-- MIGRATION : Registre dynamique des modules de la plateforme
-- ════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS `cds_modules` (
...
-- ════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS `CDS_depenses_templates` (
...
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `template_id` VARCHAR(32) DEFAULT NULL;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `fournisseur` VARCHAR(200) DEFAULT NULL;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `reference` VARCHAR(120) DEFAULT NULL;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `code_tva_fournisseur` VARCHAR(80) DEFAULT NULL;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `montant_ht`  DECIMAL(14,3) DEFAULT 0;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `montant_tva` DECIMAL(14,3) DEFAULT 0;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `timbre`      DECIMAL(14,3) DEFAULT 0;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `montant_ttc` DECIMAL(14,3) DEFAULT 0;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `lignes_json` LONGTEXT DEFAULT NULL;

-- ════════════════════════════════════════════════════════════
-- MIGRATION : Fiche de paie — infos employé + lien dépense/membre
-- ════════════════════════════════════════════════════════════
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `cin`                 VARCHAR(20)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `matricule`           VARCHAR(40)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `n_cnss`              VARCHAR(40)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `situation_familiale` VARCHAR(20)  DEFAULT 'Célibataire';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `enfants_charge`      INT          DEFAULT 0;
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `adresse`             VARCHAR(300) DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `echelon`             VARCHAR(40)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `categorie_emploi`    VARCHAR(40)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `emploi`              VARCHAR(120) DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `banque`              VARCHAR(120) DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `rib`                 VARCHAR(40)  DEFAULT '';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `mode_paiement`       VARCHAR(30)  DEFAULT 'Virement';
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `salaire_base`        DECIMAL(12,3) DEFAULT 0;

-- Lier une dépense salaire à un membre (pour générer fiche de paie + historique)
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `employe_id`    VARCHAR(32) DEFAULT NULL;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `paie_mois`     VARCHAR(7)  DEFAULT NULL;
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `paie_snapshot` LONGTEXT    DEFAULT NULL;

-- Remboursement : dépensé par, remboursé par, statut
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `depense_par`          VARCHAR(120) DEFAULT NULL COMMENT 'Nom de la personne qui a payé';
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `rembourse_par`        VARCHAR(120) DEFAULT NULL COMMENT 'Admin qui a remboursé';
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `remboursement_statut` VARCHAR(30)  DEFAULT NULL COMMENT 'demande|approuve|refuse|rembourse';
ALTER TABLE `CDS_depenses` ADD COLUMN IF NOT EXISTS `remboursement_date`   DATETIME     DEFAULT NULL;
...
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `location_type`        VARCHAR(20)   DEFAULT 'Bureau' COMMENT 'Bureau|Chantier|Administration';
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `location_zone`        VARCHAR(120)  DEFAULT '';
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `heures_estimees`      DECIMAL(6,2)  DEFAULT 0;
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `heures_reelles`       DECIMAL(6,2)  DEFAULT 0;
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `progression_planifiee` INT          DEFAULT 0 COMMENT '% planifié vs réel';
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `progression_manuelle` TINYINT(1)    DEFAULT 0 COMMENT '1 = progression forcée manuellement, ignorer cascade';
ALTER TABLE `CDS_taches` ADD COLUMN IF NOT EXISTS `assignees`             TEXT          DEFAULT NULL COMMENT 'JSON array des membres assignés (multi-affectation)';
ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `archived`              TINYINT(1)    NOT NULL DEFAULT 0 COMMENT '1 = projet archivé';
ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `archived_at`           DATETIME      DEFAULT NULL;
ALTER TABLE `CDS_projets` ADD COLUMN IF NOT EXISTS `archived_by`           VARCHAR(120)  DEFAULT NULL;

-- Couleur d'identification membre (badges kanban, avatars)
ALTER TABLE `cds_users` ADD COLUMN IF NOT EXISTS `color` VARCHAR(9) DEFAULT '#c8a96e';

-- Workflow demandes administratives — statuts avancés + pièces jointes
ALTER TABLE `CDS_demandes_admin` ADD COLUMN IF NOT EXISTS `justificatif_url`   VARCHAR(500) DEFAULT NULL;
ALTER TABLE `CDS_demandes_admin` ADD COLUMN IF NOT EXISTS `date_depot`         DATE         DEFAULT NULL;
ALTER TABLE `CDS_demandes_admin` ADD COLUMN IF NOT EXISTS `documents_manquants` LONGTEXT    DEFAULT NULL COMMENT 'JSON: checklist pièces à fournir';
ALTER TABLE `CDS_demandes_admin` ADD COLUMN IF NOT EXISTS `reponse_type`       VARCHAR(20)  DEFAULT NULL COMMENT 'positive|negative';
ALTER TABLE `CDS_demandes_admin` ADD COLUMN IF NOT EXISTS `parent_demande_id`  VARCHAR(32)  DEFAULT NULL COMMENT 'FK sur demande initiale si redépôt';

-- Timesheet — saisie du temps par collaborateur / tâche
CREATE TABLE IF NOT EXISTS `CDS_timesheets` (
  `id`          VARCHAR(32)   NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `CDS_task_dependencies` (
  `id`         VARCHAR(32)  NOT NULL PRIMARY KEY,
...
-- ════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS `CDS_leave_requests` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `user_id`          VARCHAR(32)   NOT NULL,
  `user_name`        VARCHAR(120)  DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_leave_balances` (
  `user_id`          VARCHAR(32)   NOT NULL,
  `annee`            INT           NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantiers` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `projet_id`        VARCHAR(32)   NOT NULL,
  `nom`              VARCHAR(200)  NOT NULL,
  `code`             VARCHAR(30)   DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_lots` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_taches` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_tache_deps` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_journal` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
  `date_jour`        DATE          NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_effectifs` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_intervenants` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_reunions` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_reunion_actions` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_photos` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
...
-- Réserves / Punch list
CREATE TABLE IF NOT EXISTS `CDS_chantier_reserves` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
  `lot_id`           VARCHAR(32)   DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_rfi` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
  `numero`           INT           NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_visas` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
  `lot_id`           VARCHAR(32)   DEFAULT NULL,
  `numero`           INT           NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_incidents` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_chantier_inspections` (
  `id`               VARCHAR(32)   NOT NULL PRIMARY KEY,
  `chantier_id`      VARCHAR(32)   NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_notification_prefs` (
  `user_id`       VARCHAR(32)  NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CDS_push_subscriptions` (
  `id`         VARCHAR(32)  NOT NULL PRIMARY KEY,
  `user_id`    VARCHAR(32)  NOT NULL,
