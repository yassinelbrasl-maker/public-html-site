...
function ensureMemberActivityLogTable() {
    ...
    if ($done) return;
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_member_activity_log` (
...
function logMemberActivity($userId, $userName, $action, $details = null) {
    try {
        ...
        $db = getDB();
        $db->prepare("INSERT INTO CDS_member_activity_log (id, user_id, user_name, action, details, ip_address)
                      VALUES (?, ?, ?, ?, ?, ?)")
...
function getRestrictions() {
    ...
    try {
        $db = getDB();
        $stmt = $db->prepare("SELECT setting_value FROM CDS_settings WHERE setting_key = 'cfg_restrictions' LIMIT 1");
        $stmt->execute();
