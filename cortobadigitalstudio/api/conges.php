...
try {
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_leave_requests` (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_leave_balances` (
        `user_id` VARCHAR(32) NOT NULL,
        `annee` INT NOT NULL,
    ...
    // ── Jours fériés ──
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_jours_feries` (
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM CDS_jours_feries LIKE 'paye'")->fetch();
        if (!$col) {
            $db->exec("ALTER TABLE CDS_jours_feries ADD COLUMN `paye` TINYINT(1) NOT NULL DEFAULT 1 AFTER `pont`");
        }
    } catch (\Throwable $e) { /* silencieux */ }

    // Migration unique : remplacer les anciens jours fériés algériens par les tunisiens
    $yr = (int)date('Y');
    $hasOld = $db->query("SELECT COUNT(*) FROM CDS_jours_feries WHERE YEAR(date) = $yr AND libelle LIKE '%amazigh%' OR libelle LIKE '%Victoire (19 mars%' OR libelle LIKE '%Moharram%' OR libelle LIKE '%Mawlid%'")->fetchColumn();
    if ($hasOld) {
        $db->exec("DELETE FROM CDS_jours_feries WHERE YEAR(date) = $yr");
    }
    ...
    $cnt = $db->query("SELECT COUNT(*) FROM CDS_jours_feries WHERE YEAR(date) = $yr")->fetchColumn();
    if (!$cnt) {
        $feries = [
        ...
        $ins = $db->prepare("INSERT IGNORE INTO CDS_jours_feries (date, libelle, paye) VALUES (?, ?, ?)");
...
try {
    $col = $db->query("SHOW COLUMNS FROM CDS_leave_requests LIKE 'partage'")->fetch();
    if (!$col) {
        $db->exec("ALTER TABLE CDS_leave_requests ADD COLUMN `partage` TINYINT(1) NOT NULL DEFAULT 1 AFTER `decision_at`");
    }
} catch (\Throwable $e) { /* silencieux */ }

// Migration idempotente : colonne justificatif_url
try {
    $col = $db->query("SHOW COLUMNS FROM CDS_leave_requests LIKE 'justificatif_url'")->fetch();
    if (!$col) {
        $db->exec("ALTER TABLE CDS_leave_requests ADD COLUMN `justificatif_url` VARCHAR(500) DEFAULT NULL AFTER `partage`");
    }
} catch (\Throwable $e) { /* silencieux */ }
...
function checkHolidayReminder(PDO $db): void {
    ...
    try {
        $stmt = $db->prepare("SELECT COUNT(*) FROM CDS_notifications WHERE type = 'holiday_reminder' AND link_id = ?");
        $stmt->execute([$refId]);
    ...
    $feriesCount = 0;
    try {
        $stmt = $db->prepare("SELECT COUNT(*) FROM CDS_jours_feries WHERE YEAR(date) = ?");
    ...
    $admins = [];
    try {
        $q = $db->query("SELECT id FROM cds_users WHERE LOWER(role) IN ('admin','gerant','gérant','manager','directeur') AND statut <> 'Inactif'");
        foreach ($q->fetchAll(PDO::FETCH_COLUMN) as $aid) $admins[$aid] = true;
    } catch (\Throwable $e) {}
    try {
        $q2 = $db->query("SELECT id FROM CDS_accounts WHERE role = 'admin' AND approved = 1");
...
function isManager(array $u): bool {
    ...
    if (empty($u['isMember'])) return true; // comptes CDS_accounts = admins
    return false;
}
...
function workingDays(string $d1, string $d2, ?PDO $db = null): float {
    ...
    if ($db) {
        try {
            $stmt = $db->prepare("SELECT date FROM CDS_jours_feries WHERE date BETWEEN ? AND ?");
            $stmt->execute([$d1, $d2]);
...
function ensureBalance(PDO $db, string $userId, int $annee): array {
    $stmt = $db->prepare('SELECT * FROM CDS_leave_balances WHERE user_id = ? AND annee = ?');
    $stmt->execute([$userId, $annee]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) {
        $db->prepare('INSERT INTO CDS_leave_balances (user_id, annee, conges_annuels, maladie, recuperation) VALUES (?,?,?,?,?)')
    ...
    switch ($action) {
        ...
        case 'list': {
            if (isManager($user)) {
                ...
                $stmt = $db->prepare("SELECT * FROM CDS_leave_requests WHERE $where ORDER BY
                                      FIELD(statut,'En attente','Approuvé','Refusé','Annulé'), date_debut DESC");
                $stmt->execute($params);
            } else {
                $stmt = $db->prepare('SELECT * FROM CDS_leave_requests WHERE user_id = ? ORDER BY cree_at DESC');
        ...
        case 'balances': {
            ...
            try {
                $users = $db->query("SELECT id, CONCAT(prenom,' ',nom) AS name FROM cds_users WHERE statut <> 'Inactif'")->fetchAll(PDO::FETCH_ASSOC);
            } catch (\Throwable $e) { $users = []; }
            $out = [];
            foreach ($users as $u) {
                $bal = ensureBalance($db, $u['id'], $annee);
                $stmt = $db->prepare("SELECT COALESCE(SUM(jours),0) FROM CDS_leave_requests WHERE user_id = ? AND YEAR(date_debut) = ? AND statut = 'Approuvé' AND type = 'Congés annuels'");
        ...
        case 'heatmap': {
            ...
            if ($uid !== $user['id']) {
                try {
                    $stmt = $db->prepare("SELECT CONCAT(prenom,' ',nom) FROM cds_users WHERE id = ?");
                    $stmt->execute([$uid]);
        ...
        case 'create': {
            ...
            $db->prepare("INSERT INTO CDS_leave_requests
        ...
        case 'decide': {
            ...
            $stmt = $db->prepare('SELECT * FROM CDS_leave_requests WHERE id = ?');
            $stmt->execute([$id]);
            ...
            if ($isModification && $previousStatut === 'Approuvé') {
                ...
                if ($col) {
                    ...
                    $db->prepare("UPDATE CDS_leave_balances SET $col = $col + ? WHERE user_id = ? AND annee = ?")
                       ->execute([$req['jours'], $req['user_id'], $annee]);
                }
            }

            $db->prepare('UPDATE CDS_leave_requests SET statut=?, commentaire_admin=?, decision_par=?, decision_at=NOW(), partage=? WHERE id=?')
            ...
            if ($decision === 'approve') {
                ...
                if ($col) {
                    ensureBalance($db, $req['user_id'], $annee);
                    $db->prepare("UPDATE CDS_leave_balances SET $col = GREATEST(0, $col - ?) WHERE user_id = ? AND annee = ?")
                       ->execute([$req['jours'], $req['user_id'], $annee]);
                }
            }
        ...
        case 'cancel': {
            ...
            $db->prepare("UPDATE CDS_leave_requests SET statut='Annulé', decision_par=?, decision_at=NOW() WHERE id=?")
        ...
        case 'balance_set': {
            ...
            $db->prepare("UPDATE CDS_leave_balances SET ".implode(',', $fields)." WHERE user_id = ? AND annee = ?")
               ->execute($params);
        ...
        case 'holidays_list': {
            $annee = intval($_GET['annee'] ?? date('Y'));
            $stmt = $db->prepare("SELECT * FROM CDS_jours_feries WHERE YEAR(date) = ? ORDER BY date ASC");
            $stmt->execute([$annee]);
            jsonOk($stmt->fetchAll(PDO::FETCH_ASSOC));
            break;
        }
        ...
        case 'holidays_save': {
            ...
            $id = $body['id'] ?? null;
            if ($id) {
                $db->prepare("UPDATE CDS_jours_feries SET date=?, libelle=?, pont=?, paye=? WHERE id=?")
                   ->execute([$date, $libelle, $pont, $paye, $id]);
            } else {
                $db->prepare("INSERT INTO CDS_jours_feries (date, libelle, pont, paye) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE libelle=VALUES(libelle), pont=VALUES(pont), paye=VALUES(paye)")
        ...
        case 'delete': {
            ...
            if (!moveToCorbeille($db, 'CDS_leave_requests', $id, $label, $user['name'] ?? 'unknown')) {
        ...
        case 'holidays_delete': {
            ...
            $id = $_GET['id'] ?? '';
            if (!$id) jsonError('ID requis');
            $db->prepare("DELETE FROM CDS_jours_feries WHERE id = ?")->execute([$id]);
            jsonOk(['deleted' => true]);
            break;
        }
        ...
        case 'team_calendar': {
            ...
            // Jours fériés
            $hStmt = $db->prepare("SELECT id, date, libelle, pont FROM CDS_jours_feries WHERE date BETWEEN ? AND ?");
            $hStmt->execute([$from, $to]);
            $holidays = $hStmt->fetchAll(PDO::FETCH_ASSOC);
            // Membres actifs
            $members = [];
            try {
                $members = $db->query("SELECT id, CONCAT(prenom,' ',nom) AS name FROM cds_users WHERE statut <> 'Inactif' ORDER BY prenom ASC")->fetchAll(PDO::FETCH_ASSOC);
            } catch (\Throwable $e) {}
        ...
        case 'upload_justif': {
            ...
            $db->prepare("UPDATE CDS_leave_requests SET justificatif_url = ? WHERE id = ?")->execute([$url, $reqId]);
