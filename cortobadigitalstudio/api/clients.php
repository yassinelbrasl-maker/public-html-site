...
try {
    ...
    if (!in_array('modifie_par', $cols)) {
        $db->exec("ALTER TABLE CDS_clients ADD COLUMN modifie_par VARCHAR(120) DEFAULT NULL");
    }
    if (!in_array('cree_par', $cols)) {
        $db->exec("ALTER TABLE CDS_clients ADD COLUMN cree_par VARCHAR(120) DEFAULT NULL");
    }
    if (!in_array('cree_at', $cols)) {
        $db->exec("ALTER TABLE CDS_clients ADD COLUMN cree_at DATETIME DEFAULT CURRENT_TIMESTAMP");
    }
    if (!in_array('groupe_json', $cols)) {
        $db->exec("ALTER TABLE CDS_clients ADD COLUMN groupe_json LONGTEXT DEFAULT NULL");
    }
    if (!in_array('cin', $cols)) {
        $db->exec("ALTER TABLE CDS_clients ADD COLUMN cin VARCHAR(50) DEFAULT NULL");
    }
    if (!in_array('date_cin', $cols)) {
        $db->exec("ALTER TABLE CDS_clients ADD COLUMN date_cin DATE DEFAULT NULL");
    }
    if (!in_array('mf', $cols)) {
    ...
    $typeCol = $db->query("SHOW COLUMNS FROM CDS_clients WHERE Field = 'type'")->fetch();
    if ($typeCol && strpos($typeCol['Type'], 'groupe') === false) {
        $db->exec("ALTER TABLE CDS_clients MODIFY COLUMN `type` ENUM('physique','morale','groupe') NOT NULL DEFAULT 'physique'");
    }
...
function cleanupCommas() {
    $db = getDB();
    // 1. Supprimer virgules + forcer majuscules dans CDS_clients
    $db->exec("UPDATE CDS_clients SET display_nom = UPPER(REPLACE(display_nom, ',', ''))");
    $db->exec("UPDATE CDS_clients SET nom = UPPER(REPLACE(nom, ',', ''))");
    $db->exec("UPDATE CDS_clients SET prenom = UPPER(REPLACE(prenom, ',', ''))");
    $db->exec("UPDATE CDS_clients SET raison = UPPER(REPLACE(raison, ',', ''))");
    // 2. Nettoyer les contacts auxiliaires
    $db->exec("UPDATE CDS_clients_contacts_aux SET nom = UPPER(REPLACE(nom, ',', ''))");
    $db->exec("UPDATE CDS_clients_contacts_aux SET prenom = UPPER(REPLACE(prenom, ',', ''))");
    // 3. Synchroniser les noms clients dans les projets
    $stmt = $db->query("SELECT id, code, display_nom FROM CDS_clients");
    $clients = $stmt->fetchAll(\PDO::FETCH_ASSOC);
    $upd = $db->prepare("UPDATE CDS_projets SET client = ? WHERE client_code = ?");
    ...
    $db->exec("UPDATE CDS_projets SET client = UPPER(REPLACE(client, ',', ''))");
    // 5. Forcer majuscules dans les devis si la table existe
    try {
        $db->exec("UPDATE CDS_devis SET client = UPPER(REPLACE(client, ',', ''))");
...
function getAll() {
    ...
    $stmt = $db->prepare('SELECT * FROM CDS_clients WHERE ' . implode(' AND ', $where) . ' ORDER BY cree_at DESC');
    $stmt->execute($params);
...
function getOne(string $id) {
    $db   = getDB();
    $stmt = $db->prepare('SELECT * FROM CDS_clients WHERE id = ?');
    $stmt->execute([$id]);
    $c = $stmt->fetch();
...
function create(array $user) {
    ...
    try {
        ...
        try {
            $db->prepare('
                INSERT INTO CDS_clients (id, code, num_client, type, prenom, nom, raison, matricule, mf,
...
function update($id, array $user) {
    ...
    $stOldCl = $db->prepare('SELECT code, display_nom FROM CDS_clients WHERE id = ?');
    $stOldCl->execute([$id]);
    $oldClient = $stOldCl->fetch(\PDO::FETCH_ASSOC);

    try {
        $db->prepare('
            UPDATE CDS_clients SET
    ...
    if ($oldCode && ($newDisplayNom || $newCode)) {
        $db->prepare('UPDATE CDS_projets SET client = ?, client_code = ? WHERE client_code = ?')
...
function patch($id, array $user) {
    ...
    try {
        $db->prepare('UPDATE CDS_clients SET ' . implode(', ', $sets) . ' WHERE id=?')->execute($params);
...
function remove($id, array $user) {
    ...
    $db = getDB();
    // Récupérer le nom du client pour le label
    $stmt = $db->prepare('SELECT display_nom FROM CDS_clients WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    $label = $row ? ($row['display_nom'] ?: 'Client') : 'Client';
    if (!moveToCorbeille($db, 'CDS_clients', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    $db->prepare('DELETE FROM CDS_clients_contacts_aux WHERE client_id = ?')->execute([$id]);
    jsonOk(['deleted' => $id]);
}

function getContactsAux(string $clientId) {
    $stmt = getDB()->prepare('SELECT prenom, nom, email, tel FROM CDS_clients_contacts_aux WHERE client_id = ?');
...
function saveContactsAux(string $clientId, array $contacts) {
    $db = getDB();
    $db->prepare('DELETE FROM CDS_clients_contacts_aux WHERE client_id = ?')->execute([$clientId]);
    $stmt = $db->prepare('INSERT INTO CDS_clients_contacts_aux (client_id, prenom, nom, email, tel) VALUES (?,?,?,?,?)');
