...
function create(array $user) {
    ...
    $db->prepare('
        INSERT INTO CDS_journal (id, tache_id, projet_id, membre, date_jour,
    ...
    if ($progApres > 0) {
        $db->prepare('UPDATE CDS_taches SET progression = ?, statut = ? WHERE id = ?')
           ->execute([
               $progApres,
...
function update($id, array $user) {
    ...
    $stmt = $db->prepare('SELECT id, tache_id FROM CDS_journal WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch();
    ...
    $params[] = $id;
    $db->prepare('UPDATE CDS_journal SET ' . implode(', ', $fields) . ' WHERE id = ?')->execute($params);
...
function remove($id, array $user) {
    if (!$id) jsonError('ID requis');
    $db = getDB();
    $db->prepare('DELETE FROM CDS_journal WHERE id = ?')->execute([$id]);
    jsonOk(['deleted' => $id]);
}
...
function recalcParent($db, $childId) {
    ...
    $stmt2 = $db->prepare('SELECT AVG(progression) AS avg_prog FROM CDS_taches WHERE parent_id = ?');
    $stmt2->execute([$parentId]);
    $r = $stmt2->fetch();
    $avg = $r ? intval(round($r['avg_prog'])) : 0;
    $db->prepare('UPDATE CDS_taches SET progression = ? WHERE id = ?')->execute([$avg, $parentId]);
}
