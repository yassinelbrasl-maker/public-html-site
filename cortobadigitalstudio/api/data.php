...
function createRow($cfg, $user) {
    $table = $cfg['table'];
    $tableRestrMap = ['CDS_devis' => 'creer_devis', 'CDS_factures' => 'creer_factures'];
    if (isset($tableRestrMap[$table]) && isRestricted($user, $tableRestrMap[$table])) {
        $label = ($table === 'CDS_devis') ? 'devis' : 'factures';
        jsonError('Vous n\'êtes pas autorisé à créer des ' . $label, 403);
    }
...
function handleParametres($method, $user) {
    $db = getDB();
    if ($method === 'GET') {
        $stmt = $db->query('SELECT cle, valeur FROM CDS_parametres');
        $out  = array();
    ...
    if ($method === 'POST' || $method === 'PUT') {
        $body = getBody();
        $stmt = $db->prepare('INSERT INTO CDS_parametres (cle, valeur) VALUES (?,?) ON DUPLICATE KEY UPDATE valeur=?');
    ...
    if ($method === 'DELETE') {
        ...
        if ($key) { $db->prepare('DELETE FROM CDS_parametres WHERE cle=?')->execute(array($key)); }
