...
function ensureModulesTable() {
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `cds_modules` (
    ...
    $count = (int)$db->query("SELECT COUNT(*) FROM cds_modules")->fetchColumn();
    if ($count === 0) {
        $defaults = array(
        ...
        $stmt = $db->prepare("INSERT INTO cds_modules (id, label, route_url, categorie, ordre) VALUES (?, ?, ?, ?, ?)");
    ...
    $up = $db->prepare("INSERT INTO cds_modules (id, label, route_url, categorie, ordre)
                        VALUES (?, ?, ?, ?, ?)
...
try {
    ...
    if ($method === 'GET') {
        $rows = $db->query("SELECT id, label, route_url, categorie, ordre
                            FROM cds_modules WHERE actif = 1 ORDER BY ordre ASC, label ASC")
        ...
        $stmt = $db->prepare("INSERT INTO cds_modules (id, label, route_url, categorie, ordre)
                              VALUES (?, ?, ?, ?, ?)
        ...
        $stmt = $db->prepare("UPDATE cds_modules SET actif = 0 WHERE id = ?");
