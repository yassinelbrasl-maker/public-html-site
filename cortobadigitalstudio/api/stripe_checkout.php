...
function getStripeConfig() {
    $db = getDB();
    $s = $db->prepare("SELECT valeur FROM CDS_parametres WHERE cle = 'stripe_config'");
    $s->execute();
    $raw = $s->fetchColumn();
...
function createSession($user) {
    ...
    $s = $db->prepare("SELECT f.*, c.display_nom AS client_nom, c.email AS client_email, p.code AS projet_code
        ...
        LEFT JOIN CDS_projets p ON p.id = f.projet_id
        WHERE f.id = ?");
    $s->execute([$factureId]);
...
function handleWebhook() {
    ...
    if ($event['type'] === 'checkout.session.completed') {
        ...
        $s = $db->prepare("SELECT id FROM CDS_paiements WHERE stripe_session_id = ?");
        $s->execute([$sessionId]);
        if ($s->fetchColumn()) {
...
function getPaymentStatus() {
    ...
    $s = $db->prepare("SELECT p.*, f.numero AS facture_numero, f.statut AS facture_statut
        FROM CDS_paiements p LEFT JOIN CDS_factures f ON f.id = p.facture_id
