...
function devisToFacture($user) {
    ...
    try {
        $clientEmail = '';
        if ($devis['client_id']) {
            $sc = $db->prepare("SELECT email FROM CDS_clients WHERE id = ?");
            $sc->execute([$devis['client_id']]);
...
function getProjetSolde() {
    ...
    // Project info
    $s = $db->prepare("SELECT id, code, nom, client, honoraires_prevus, honoraires_factures, honoraires_encaisses FROM CDS_projets WHERE id = ?");
    $s->execute([$projetId]);
    ...
    // Total invoiced
    $s = $db->prepare("SELECT COALESCE(SUM(COALESCE(net_payer, montant_ttc, 0)),0) FROM CDS_factures WHERE projet_id = ? AND statut != 'Annulée'");
    $s->execute([$projetId]);
    $totalFacture = (float)$s->fetchColumn();

    // Total paid
    $s = $db->prepare("SELECT COALESCE(SUM(p.montant),0) FROM CDS_paiements p JOIN CDS_factures f ON f.id = p.facture_id WHERE f.projet_id = ?");
    $s->execute([$projetId]);
    ...
    $s = $db->prepare("SELECT p.id, p.montant, p.date_paiement, p.mode_paiement, p.reference, f.numero AS facture_numero
        ...
        JOIN CDS_factures f ON f.id = p.facture_id
        WHERE f.projet_id = ?
...
function generatePDF() {
    ...
    // Ensure CDS_clients has mf column
    try {
    ...
    try {
        $s = $db->prepare("SELECT valeur FROM CDS_parametres WHERE cle = 'entreprise'");
        $s->execute();
        $raw = $s->fetchColumn();
        if ($raw) {
    ...
    switch ($type) {
        case 'devis':
            $s = $db->prepare("SELECT d.*, c.display_nom AS client_nom, c.adresse AS client_adresse, c.mf AS client_mf, c.email AS client_email
                FROM CDS_devis d LEFT JOIN CDS_clients c ON c.id = d.client_id WHERE d.id = ?");
            $s->execute([$id]);
        ...
        case 'facture':
            $s = $db->prepare("SELECT f.*, c.display_nom AS client_nom_full, c.email AS client_email
                FROM CDS_factures f LEFT JOIN CDS_clients c ON c.id = f.client_id WHERE f.id = ?");
            $s->execute([$id]);
            $doc = $s->fetch(\PDO::FETCH_ASSOC);
        ...
        case 'recu':
            ...
            $s = $db->prepare("
                ...
                FROM CDS_paiements p
                LEFT JOIN CDS_factures f ON f.id = p.facture_id
                LEFT JOIN CDS_clients c ON c.id = p.client_id
                LEFT JOIN CDS_projets pr ON pr.id = p.projet_id
                WHERE p.id = ?
            ");
...
function sendDocument($user) {
    ...
    switch ($type) {
        case 'devis':
            $s = $db->prepare("SELECT numero, objet, montant_ttc FROM CDS_devis WHERE id = ?");
            $s->execute([$id]);
        ...
        case 'facture':
            $s = $db->prepare("SELECT numero, objet, net_payer, montant_ttc, montant_ht, tva, fodec, timbre,
                               ...
                               lignes_json, montant_lettres FROM CDS_factures WHERE id = ?");
            $s->execute([$id]);
            $d = $s->fetch(\PDO::FETCH_ASSOC);
            ...
            break;
        case 'recu':
            $s = $db->prepare("SELECT p.montant, f.numero FROM CDS_paiements p LEFT JOIN CDS_factures f ON f.id = p.facture_id WHERE p.id = ?");
            $s->execute([$id]);
            $d = $s->fetch(\PDO::FETCH_ASSOC);
...
function publishInvoiceToPortal($user) {
    ...
    $s = $db->prepare("SELECT id, numero, objet, client_id, projet_id, net_payer, montant_ttc,
                        montant_ht, tva, date_echeance, mission_phase FROM CDS_factures WHERE id = ?");
    $s->execute([$factureId]);
    ...
    $check = $db->prepare("SELECT id FROM CDS_client_documents WHERE client_id = ? AND projet_id = ? AND titre = ? AND statut != 'Remplace'");
    $check->execute([$clientId, $projetId, $titre]);
    if ($check->fetch()) {
        jsonError('Cette facture est déjà publiée au portail client');
    }

    // Créer l'entrée dans CDS_client_documents
