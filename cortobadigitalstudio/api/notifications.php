...
try {
    $_db0 = getDB();
    $_db0->exec("CREATE TABLE IF NOT EXISTS `CDS_notifications` (
    ...
    try { $_db0->exec("ALTER TABLE CDS_notifications ADD COLUMN IF NOT EXISTS `is_archived` TINYINT(1) NOT NULL DEFAULT 0 AFTER `is_read`"); }
...
if (!function_exists('notifCreate')) {
    function notifCreate(PDO $db, string $userId, string $type, string $title,
        ...
        $db->prepare("INSERT INTO CDS_notifications
...
try {
    switch ($action) {
        case 'list': {
            ...
            $stmt = $db->prepare("SELECT * FROM CDS_notifications WHERE $where ORDER BY $orderBy LIMIT $limit");
            $stmt->execute($params);
        ...
        case 'count': {
            $stmt = $db->prepare("SELECT COUNT(*) FROM CDS_notifications WHERE user_id = ? AND is_read = 0 AND is_archived = 0");
        ...
        case 'types': {
            $stmt = $db->prepare("SELECT DISTINCT type FROM CDS_notifications WHERE user_id = ? ORDER BY type");
            $stmt->execute([$user['id']]);
        ...
        case 'mark_read': {
            ...
            $db->prepare("UPDATE CDS_notifications SET is_read = 1 WHERE id = ? AND user_id = ?")
               ->execute([$id, $user['id']]);
            jsonOk(['id' => $id]);
            break;
        }
        ...
        case 'mark_unread': {
            $id = $_GET['id'] ?? '';
            if (!$id) jsonError('ID requis');
            $db->prepare("UPDATE CDS_notifications SET is_read = 0 WHERE id = ? AND user_id = ?")
               ->execute([$id, $user['id']]);
            jsonOk(['id' => $id]);
            break;
        }
        ...
        case 'mark_all_read': {
            $db->prepare("UPDATE CDS_notifications SET is_read = 1 WHERE user_id = ? AND is_archived = 0")
               ->execute([$user['id']]);
            jsonOk(['ok' => true]);
            break;
        }
        ...
        case 'archive': {
            $id = $_GET['id'] ?? '';
            if (!$id) jsonError('ID requis');
            $db->prepare("UPDATE CDS_notifications SET is_archived = 1 WHERE id = ? AND user_id = ?")
               ->execute([$id, $user['id']]);
            jsonOk(['id' => $id]);
            break;
        }
        ...
        case 'unarchive': {
            $id = $_GET['id'] ?? '';
            if (!$id) jsonError('ID requis');
            $db->prepare("UPDATE CDS_notifications SET is_archived = 0 WHERE id = ? AND user_id = ?")
               ->execute([$id, $user['id']]);
            jsonOk(['id' => $id]);
            break;
        }
        ...
        case 'delete': {
            $id = $_GET['id'] ?? '';
            if (!$id) jsonError('ID requis');
            $db->prepare("DELETE FROM CDS_notifications WHERE id = ? AND user_id = ?")
               ->execute([$id, $user['id']]);
            jsonOk(['id' => $id]);
            break;
        }
