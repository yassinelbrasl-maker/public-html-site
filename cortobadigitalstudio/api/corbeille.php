...
// ── Auto-création de la table CDS_corbeille ──────────────────────────────────
if (!function_exists('moveToCorbeille')) {
    try {
        $__cdb = getDB();
        $__cdb->exec("CREATE TABLE IF NOT EXISTS `CDS_corbeille` (
    ...
    function moveToCorbeille(\PDO $db, string $table, string $id, string $label, string $userName): bool {
        try {
            ...
            $db->prepare('
                INSERT INTO CDS_corbeille (id, table_source, item_id, item_data, label, deleted_by)
                VALUES (?, ?, ?, ?, ?, ?)
            ')->execute([
                $corbeilleId,
                $table,
...
function corbeille_getAll() {
    $db   = getDB();
    $stmt = $db->query('SELECT * FROM CDS_corbeille ORDER BY deleted_at DESC');
...
function corbeille_restore($id, array $user) {
    ...
    $db   = getDB();
    $stmt = $db->prepare('SELECT * FROM CDS_corbeille WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    $db->prepare('DELETE FROM CDS_corbeille WHERE id = ?')->execute([$id]);
...
function corbeille_permanentDelete($id, array $user) {
    ...
    $db = getDB();
    $stmt = $db->prepare('SELECT id FROM CDS_corbeille WHERE id = ?');
    $stmt->execute([$id]);
...
function corbeille_purge(array $user) {
    ...
    $db   = getDB();
    $stmt = $db->prepare('DELETE FROM CDS_corbeille WHERE deleted_at < DATE_SUB(NOW(), INTERVAL 30 DAY)');
    $stmt->execute();
