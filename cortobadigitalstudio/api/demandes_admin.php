...
try {
    ...
    // S'assurer que CDS_taches a la colonne demande_admin_id
    try { $db0->exec("ALTER TABLE CDS_taches ADD COLUMN IF NOT EXISTS demande_admin_id VARCHAR(32) DEFAULT NULL COMMENT 'Lien vers CDS_demandes_admin'"); }
    catch (\Throwable $e) {}
    foreach ($extra as $cdef) {
        try { $db0->exec("ALTER TABLE CDS_demandes_admin ADD COLUMN IF NOT EXISTS $cdef"); }
        catch (\Throwable $e) {}
    }
} catch (\Throwable $e) {}
...
function cloneDemande($id, $user) {
    ...
    $db = getDB();
    $stmt = $db->prepare('SELECT * FROM CDS_demandes_admin WHERE id = ?');
    $stmt->execute([$id]);
    $src = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$src) jsonError('Demande introuvable', 404);

    $newId = bin2hex(random_bytes(16));
    $db->prepare('INSERT INTO CDS_demandes_admin
...
function create(array $user) {
    ...
    $db->prepare('
        INSERT INTO CDS_demandes_admin (id, projet_id, client_id, type_demande, langue, administration,
    ...
    if ($projetId) {
        ...
        $stOrd = $db->prepare('SELECT COALESCE(MAX(ordre),0)+1 AS next_ord FROM CDS_taches WHERE projet_id = ? AND parent_id IS NULL');
...
function update($id, array $user) {
    ...
    $stmt = $db->prepare('SELECT id FROM CDS_demandes_admin WHERE id = ?');
    $stmt->execute([$id]);
    ...
    $params[] = $id;
    $db->prepare('UPDATE CDS_demandes_admin SET ' . implode(', ', $fields) . ' WHERE id = ?')
       ->execute($params);
...
function remove($id, array $user) {
    if (!$id) jsonError('ID requis');
    $db = getDB();
    $db->prepare('DELETE FROM CDS_demandes_admin WHERE id = ?')->execute([$id]);
    jsonOk(['deleted' => $id]);
}
