...
function ensureProjetsRendementColumns() {
    ...
    foreach ($extra as $def) {
        try { $db->exec("ALTER TABLE CDS_projets ADD COLUMN IF NOT EXISTS $def"); }
...
try {
    $db = getDB();
    $cols = array_column($db->query("SHOW COLUMNS FROM CDS_projets")->fetchAll(), 'Field');
    if (!in_array('modifie_par', $cols)) {
        $db->exec("ALTER TABLE CDS_projets ADD COLUMN modifie_par VARCHAR(120) DEFAULT NULL");
    }
    if (!in_array('cree_par', $cols)) {
        $db->exec("ALTER TABLE CDS_projets ADD COLUMN cree_par VARCHAR(120) DEFAULT NULL");
    }
    if (!in_array('cree_at', $cols)) {
        $db->exec("ALTER TABLE CDS_projets ADD COLUMN cree_at DATETIME DEFAULT CURRENT_TIMESTAMP");
    }
} catch (\Throwable $e) {}
...
function getAll() {
    ...
    $sql  = 'SELECT p.* FROM CDS_projets p WHERE ' . implode(' AND ', $where) . ' ORDER BY p.cree_at DESC';
    $stmt = $db->prepare($sql);
    $stmt->execute($params);
...
function create(array $user) {
    ...
    for ($attempt = 0; $attempt < $maxRetries; $attempt++) {
        try {
            ...
            if ($e->getCode() == 23000 && strpos($e->getMessage(), 'Duplicate') !== false) {
                ...
                $stmt = $db->prepare("SELECT MAX(CAST(SUBSTRING_INDEX(code, '_', 1) AS UNSIGNED)) AS mx FROM CDS_projets WHERE code LIKE ?");
                $stmt->execute(['%_' . $yy . '_%']);
    ...
    if ($code !== ($body['code'] ?? '')) {
        $db->prepare('UPDATE CDS_projets SET code = ? WHERE id = ?')->execute([$code, $id]);
    }
    ...
    if (!empty($body['clientCode'])) {
        $db->prepare('UPDATE CDS_clients SET projets = projets + 1 WHERE code = ?')->execute([$body['clientCode']]);
    }
...
function update($id, array $user) {
    ...
    $stmt = $db->prepare('SELECT id FROM CDS_projets WHERE id = ?');
    $stmt->execute([$id]);
    ...
    if ($action === 'archive' || $action === 'unarchive') {
        ...
        $db->prepare('UPDATE CDS_projets SET archived=?, archived_at=?, archived_by=?, modifie_par=? WHERE id=?')
           ->execute([
               $flag,
    ...
    if ($action === 'affect_mission' || $isMissionsOnly) {
        ...
        $db->prepare('UPDATE CDS_projets SET modifie_par=? WHERE id=?')
    ...
    if (isset($body['nasPath']) && !isset($body['nom'])) {
        $db->prepare('UPDATE CDS_projets SET nas_path=?, modifie_par=? WHERE id=?')
    ...
    if (!empty($body['create_chat_room'])) {
        try {
            $st = $db->prepare('SELECT * FROM CDS_projets WHERE id = ?');
            $st->execute([$id]);
...
function remove($id, array $user) {
    ...
    $stmt = $db->prepare('SELECT code, nom FROM CDS_projets WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['code'] ?? '') . ' ' . ($row['nom'] ?? '')) : 'Projet';
    if (!moveToCorbeille($db, 'CDS_projets', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    $db->prepare('DELETE FROM CDS_projets_missions WHERE projet_id = ?')->execute([$id]);
    $db->prepare('DELETE FROM CDS_projets_intervenants WHERE projet_id = ?')->execute([$id]);
    jsonOk(['deleted' => $id]);
}

function getMissions(string $projetId) {
    $stmt = getDB()->prepare('SELECT mission FROM CDS_projets_missions WHERE projet_id = ? ORDER BY id');
    $stmt->execute([$projetId]);
    return array_column($stmt->fetchAll(), 'mission');
}

function getIntervenants(string $projetId) {
    $stmt = getDB()->prepare('SELECT role, nom, contact FROM CDS_projets_intervenants WHERE projet_id = ? ORDER BY id');
    $stmt->execute([$projetId]);
...
function saveMissions(string $projetId, array $missions) {
    $db = getDB();
    $db->prepare('DELETE FROM CDS_projets_missions WHERE projet_id = ?')->execute([$projetId]);
    $stmt = $db->prepare('INSERT INTO CDS_projets_missions (projet_id, mission) VALUES (?,?)');
...
function saveIntervenants(string $projetId, array $intervenants) {
    $db = getDB();
    $db->prepare('DELETE FROM CDS_projets_intervenants WHERE projet_id = ?')->execute([$projetId]);
    $stmt = $db->prepare('INSERT INTO CDS_projets_intervenants (projet_id, role, nom, contact) VALUES (?,?,?,?)');
