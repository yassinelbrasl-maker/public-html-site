...
function cpaCreateAccount($user) {
    ...
    $stmt = $db->prepare("SELECT id, display_nom FROM CDS_clients WHERE id = ?");
    $stmt->execute([$clientId]);
    if (!$stmt->fetch()) jsonError('Client introuvable', 404);

    // Verifier doublon email
    $stmt = $db->prepare("SELECT id FROM CDS_client_accounts WHERE email = ?");
    $stmt->execute([$email]);
    if ($stmt->fetch()) jsonError('Email deja utilise');

    $id = cpGenId();
    $db->prepare("INSERT INTO CDS_client_accounts (id, client_id, email, password_hash, nom, statut)
                  VALUES (?, ?, ?, ?, ?, 'actif')")
...
function cpaUpdateAccount($user) {
    ...
    $db->prepare("UPDATE CDS_client_accounts SET " . implode(', ', $sets) . " WHERE id = ?")
       ->execute($params);
...
function cpaDeleteAccount($user) {
    ...
    $db = getDB();
    $db->prepare("DELETE FROM CDS_client_accounts WHERE id = ?")->execute([$id]);
...
function cpaPublishDocument($user) {
    ...
    $stmt = $db->prepare("SELECT id, version FROM CDS_client_documents
    ...
    if ($prev) {
        ...
        $db->prepare("UPDATE CDS_client_documents SET statut = 'Remplace' WHERE id = ?")->execute([$prev['id']]);
    }
...
function cpaRequestValidation($user) {
    ...
    $db = getDB();
    $id = cpGenId();
    $db->prepare("INSERT INTO CDS_client_validations
    ...
    if ($documentId) {
        $db->prepare("UPDATE CDS_client_documents SET statut = 'En attente validation' WHERE id = ?")
           ->execute([$documentId]);
    }
...
function cpaClientDocuments($user) {
    ...
    $sql = "SELECT d.*, p.code AS projet_code, p.nom AS projet_nom, c.display_nom AS client_display
            ...
            LEFT JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = d.client_id WHERE 1=1";
    $params = [];
...
function cpaClientValidations($user) {
    ...
    $sql = "SELECT v.*, d.titre AS doc_titre, p.code AS projet_code, p.nom AS projet_nom
            ...
            LEFT JOIN CDS_projets p ON p.id COLLATE utf8mb4_unicode_ci = v.projet_id WHERE 1=1";
    $params = [];
...
function cpaClientChatRooms($user) {
    ...
    $stmt = $db->prepare("
        SELECT r.id, r.name, r.projet_id, r.is_archived, r.cree_at,
               ...
               (SELECT COUNT(*) FROM CDS_chat_messages m3 WHERE m3.room_id = r.id AND m3.sender_id LIKE 'client_%'
                AND m3.cree_at > COALESCE((SELECT MAX(m4.cree_at) FROM CDS_chat_messages m4 WHERE m4.room_id = r.id AND m4.sender_id NOT LIKE 'client_%'), '1970-01-01')) AS unread_count
        FROM CDS_chat_rooms r
        LEFT JOIN CDS_projets p ON p.id COLLATE utf8mb4_unicode_ci = r.projet_id
        LEFT JOIN CDS_clients c ON c.code = p.client_code COLLATE utf8mb4_unicode_ci
        WHERE r.type = 'client'
...
function cpaClientChatMessages($user) {
    ...
    $stmt = $db->prepare("SELECT id FROM CDS_chat_rooms WHERE id = ? AND type = 'client'");
    $stmt->execute([$roomId]);
...
function cpaClientChatSend($user) {
    ...
    $db = getDB();
    $stmt = $db->prepare("SELECT id, is_archived FROM CDS_chat_rooms WHERE id = ? AND type = 'client'");
    $stmt->execute([$roomId]);
...
function cpaAccessLog($user) {
    ...
    // Total
    $stmt = $db->prepare("SELECT COUNT(*) FROM CDS_client_activity_log l $where");
    $stmt->execute($params);
    ...
    $stmt = $db->prepare("
        ...
        LEFT JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = l.client_id
        $where
    ...
    $clients = $db->query("
        ...
        FROM CDS_client_activity_log l
        JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = l.client_id
