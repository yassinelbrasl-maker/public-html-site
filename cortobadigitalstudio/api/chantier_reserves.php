...
function ensureReservesTables() {
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_reserves` (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_rfi` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
      `numero` INT NOT NULL DEFAULT 1,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_visas` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
      `lot_id` VARCHAR(32) DEFAULT NULL,
      `numero` INT NOT NULL DEFAULT 1,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_photos` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
...
function listReserves() {
    ...
    $statut = $_GET['statut'] ?? '';
    $lot = $_GET['lot_id'] ?? '';
    $sql = "SELECT r.*, l.nom AS lot_nom FROM CDS_chantier_reserves r
            LEFT JOIN CDS_chantier_lots l ON l.id = r.lot_id
            WHERE r.chantier_id=?";
    $params = [$cid];
...
function getReserve($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT r.*, l.nom AS lot_nom FROM CDS_chantier_reserves r
                          LEFT JOIN CDS_chantier_lots l ON l.id = r.lot_id WHERE r.id=?");
    $stmt->execute([$id]);
    $r = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$r) jsonError('Réserve introuvable', 404);
    // Attach photos
    $s2 = $db->prepare("SELECT * FROM CDS_chantier_photos WHERE reserve_id=? ORDER BY cree_at");
    $s2->execute([$id]);
...
function createReserve($user) {
    ...
    $cid = $b['chantier_id'] ?? '';
    // Auto-numérotation
    $s = $db->prepare("SELECT COALESCE(MAX(numero),0)+1 AS n FROM CDS_chantier_reserves WHERE chantier_id=?");
    $s->execute([$cid]);
    $num = $s->fetch(PDO::FETCH_ASSOC)['n'];

    $db->prepare("INSERT INTO CDS_chantier_reserves (id, chantier_id, lot_id, numero, titre, description,
...
function updateReserve($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_reserves SET lot_id=?, titre=?, description=?, zone=?,
...
function deleteReserve($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_reserves WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listRfi() {
    ...
    $cid = $_GET['chantier_id'] ?? '';
    $statut = $_GET['statut'] ?? '';
    $sql = "SELECT * FROM CDS_chantier_rfi WHERE chantier_id=?";
    $params = [$cid];
...
function getRfi($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM CDS_chantier_rfi WHERE id=?");
    $stmt->execute([$id]);
    $r = $stmt->fetch(PDO::FETCH_ASSOC);
...
function createRfi($user) {
    ...
    $id = bin2hex(random_bytes(16));
    $cid = $b['chantier_id'] ?? '';
    $s = $db->prepare("SELECT COALESCE(MAX(numero),0)+1 AS n FROM CDS_chantier_rfi WHERE chantier_id=?");
    $s->execute([$cid]);
    $num = $s->fetch(PDO::FETCH_ASSOC)['n'];

    $db->prepare("INSERT INTO CDS_chantier_rfi (id, chantier_id, numero, objet, description, documents_ref,
...
function updateRfi($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_rfi SET objet=?, description=?, documents_ref=?, emetteur=?,
...
function deleteRfi($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_rfi WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listVisas() {
    ...
    $cid = $_GET['chantier_id'] ?? '';
    $statut = $_GET['statut'] ?? '';
    $sql = "SELECT v.*, l.nom AS lot_nom FROM CDS_chantier_visas v
            LEFT JOIN CDS_chantier_lots l ON l.id = v.lot_id
            WHERE v.chantier_id=?";
    $params = [$cid];
...
function getVisa($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT v.*, l.nom AS lot_nom FROM CDS_chantier_visas v
                          LEFT JOIN CDS_chantier_lots l ON l.id = v.lot_id WHERE v.id=?");
    $stmt->execute([$id]);
    $r = $stmt->fetch(PDO::FETCH_ASSOC);
...
function createVisa($user) {
    ...
    $id = bin2hex(random_bytes(16));
    $cid = $b['chantier_id'] ?? '';
    $s = $db->prepare("SELECT COALESCE(MAX(numero),0)+1 AS n FROM CDS_chantier_visas WHERE chantier_id=?");
    $s->execute([$cid]);
    $num = $s->fetch(PDO::FETCH_ASSOC)['n'];

    $db->prepare("INSERT INTO CDS_chantier_visas (id, chantier_id, lot_id, numero, document_titre, document_ref,
...
function updateVisa($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_visas SET lot_id=?, document_titre=?, document_ref=?, document_url=?,
...
function deleteVisa($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_visas WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
