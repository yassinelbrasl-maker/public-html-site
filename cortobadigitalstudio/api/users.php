...
function ensureUsersTable() {
    ...
    foreach ($extraCols as $colDef) {
        try { $db->exec("ALTER TABLE cds_users ADD COLUMN IF NOT EXISTS $colDef"); }
...
try {
    ...
    if ($method === 'GET') {
        ...
        $stmt = $db->query("SELECT * FROM cds_users WHERE is_admin = 0 ORDER BY created_at ASC");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $out = array();
        ...
        if ($isEdit) {
            ...
            $vals[] = $id;
            $sql = "UPDATE cds_users SET " . implode(',', $set) . " WHERE id=?";
            ...
            $sql  = "INSERT INTO cds_users (`" . implode('`,`', $keys) . "`) VALUES (" . implode(',', $ph) . ")";
        ...
        $stmt = $db->prepare("DELETE FROM cds_users WHERE id = ? AND is_admin = 0");
