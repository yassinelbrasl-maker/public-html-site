...
function ensurePaiementsSchema() {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_paiements` (
    ...
    try { $db->exec("ALTER TABLE CDS_paiements MODIFY `facture_id` varchar(32) DEFAULT NULL"); } catch (\Throwable $e) {}
    try { $db->exec("ALTER TABLE CDS_paiements ADD COLUMN `mission_phase` varchar(120) DEFAULT NULL AFTER `client_id`"); } catch (\Throwable $e) {}

    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_relances` (
    ...
    // Ensure CDS_factures has required columns
    $fa = [
...
function recordPayment($db, $data, $userName = null) {
    ...
    if ($factureId) {
        $s = $db->prepare("SELECT id, projet_id, client_id, net_payer, montant_ttc, montant_paye, statut FROM CDS_factures WHERE id = ?");
        $s->execute([$factureId]);
    ...
    $db->prepare("INSERT INTO CDS_paiements (id, facture_id, projet_id, client_id, mission_phase, montant, date_paiement, mode_paiement, reference, stripe_session_id, stripe_payment_intent, notes, cree_par) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
       ->execute([
           $id,
           $factureId ?: null,
           $projetId,
    ...
    if ($factureId && $facture) {
        ...
        $s = $db->prepare("SELECT COALESCE(SUM(montant),0) FROM CDS_paiements WHERE facture_id = ?");
        $s->execute([$factureId]);
        ...
        $db->prepare("UPDATE CDS_factures SET montant_paye = ?, statut = ?, date_paiement = CASE WHEN ? >= ? THEN CURDATE() ELSE date_paiement END WHERE id = ?")
...
function listPaiements() {
    ...
    $stmt = $db->prepare("SELECT p.*, f.numero AS facture_numero, c.display_nom AS client_nom, pr.nom AS projet_nom, pr.code AS projet_code
        FROM CDS_paiements p
        LEFT JOIN CDS_factures f ON f.id COLLATE utf8mb4_unicode_ci = p.facture_id COLLATE utf8mb4_unicode_ci
        LEFT JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = p.client_id COLLATE utf8mb4_unicode_ci
        LEFT JOIN CDS_projets pr ON pr.id COLLATE utf8mb4_unicode_ci = p.projet_id COLLATE utf8mb4_unicode_ci
        WHERE $where
...
function createPaiement($user) {
    ...
    try {
        ...
        if (!empty($body['facture_id'])) {
            $s = $db->prepare("SELECT numero FROM CDS_factures WHERE id = ?");
            $s->execute([$body['facture_id']]);
            $numero = $s->fetchColumn();
            $contextLabel = 'facture ' . $numero;
        } else if (!empty($body['projet_id'])) {
            $s = $db->prepare("SELECT COALESCE(code, nom) FROM CDS_projets WHERE id = ?");
    ...
    $s = $db->prepare("SELECT p.*, f.numero AS facture_numero FROM CDS_paiements p LEFT JOIN CDS_factures f ON f.id COLLATE utf8mb4_unicode_ci = p.facture_id COLLATE utf8mb4_unicode_ci WHERE p.id = ?");
    $s->execute([$id]);
...
function deletePaiement($user) {
    ...
    $db = getDB();
    $s = $db->prepare("SELECT facture_id, projet_id FROM CDS_paiements WHERE id = ?");
    $s->execute([$id]);
    $paiement = $s->fetch(\PDO::FETCH_ASSOC);
    if (!$paiement) jsonError('Paiement introuvable', 404);

    $db->prepare("DELETE FROM CDS_paiements WHERE id = ?")->execute([$id]);
    ...
    $s = $db->prepare("SELECT net_payer, montant_ttc FROM CDS_factures WHERE id = ?");
    $s->execute([$factureId]);
    ...
    $db->prepare("UPDATE CDS_factures SET montant_paye = ?, statut = ? WHERE id = ?")->execute([$totalPaye, $newStatut, $factureId]);
...
function getReceivables() {
    ...
    $stmt = $db->query("SELECT
        ...
        FROM CDS_factures f
        LEFT JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = f.client_id COLLATE utf8mb4_unicode_ci
        LEFT JOIN CDS_projets p ON p.id COLLATE utf8mb4_unicode_ci = f.projet_id COLLATE utf8mb4_unicode_ci
...
function getAgedBalance() {
    ...
    $stmt = $db->query("SELECT
        ...
        FROM CDS_factures f
        JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = f.client_id COLLATE utf8mb4_unicode_ci
        WHERE f.statut IN ('Impayée', 'Partiellement payée', 'Émise')
...
function getCashflow() {
    ...
    try {
        $stmt = $db->prepare("SELECT DATE_FORMAT(date_prevue, '%Y-%m') AS mois, SUM(montant_prevu) AS total
            FROM CDS_echeancier WHERE statut = 'prevu' AND date_prevue >= ? AND date_prevue < DATE_ADD(?, INTERVAL 6 MONTH)
            GROUP BY mois");
    ...
    try {
        $stmt = $db->prepare("SELECT DATE_FORMAT(date_echeance, '%Y-%m') AS mois,
            ...
            FROM CDS_factures WHERE statut IN ('Impayée', 'Partiellement payée', 'Émise')
    ...
    try {
        $stmt = $db->prepare("SELECT DATE_FORMAT(date_paiement, '%Y-%m') AS mois, SUM(montant) AS total
            FROM CDS_paiements WHERE date_paiement >= ? AND date_paiement < DATE_ADD(?, INTERVAL 6 MONTH)
            GROUP BY mois");
        $stmt->execute([$now->format('Y-m-01'), $now->format('Y-m-01')]);
        foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $r) {
    ...
    try {
        $stmt = $db->query("SELECT montant, frequence FROM CDS_depenses_templates WHERE actif = 1");
...
function sendRelance($user) {
    ...
    $s = $db->prepare("SELECT f.*, c.display_nom AS client_nom, c.email AS client_email
        FROM CDS_factures f LEFT JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = f.client_id COLLATE utf8mb4_unicode_ci WHERE f.id = ?");
    $s->execute([$factureId]);
    $facture = $s->fetch(\PDO::FETCH_ASSOC);
    ...
    try {
        $s = $db->prepare("SELECT valeur FROM CDS_parametres WHERE cle = 'relance_templates'");
        $s->execute();
    ...
    // Log relance
    $db->prepare("INSERT INTO CDS_relances (facture_id, client_id, type, canal, niveau, email_to, template_used, statut, cree_par) VALUES (?,?,?,?,?,?,?,?,?)")
    ...
    $db->prepare("UPDATE CDS_factures SET relance_niveau = ?, derniere_relance = NOW() WHERE id = ?")
...
function getRelanceLog() {
    ...
    $stmt = $db->prepare("SELECT r.*, f.numero AS facture_numero, c.display_nom AS client_nom
        FROM CDS_relances r
        LEFT JOIN CDS_factures f ON f.id COLLATE utf8mb4_unicode_ci = r.facture_id COLLATE utf8mb4_unicode_ci
        LEFT JOIN CDS_clients c ON c.id COLLATE utf8mb4_unicode_ci = r.client_id COLLATE utf8mb4_unicode_ci
        WHERE $where
...
function getReceiptData() {
    ...
    $s2 = $db->prepare("SELECT COALESCE(SUM(montant),0) FROM CDS_paiements WHERE facture_id = ?");
