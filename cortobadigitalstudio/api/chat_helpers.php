...
function chat_ensure_schema() {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS CDS_chat_rooms (
    ...
    try { $db->exec("ALTER TABLE CDS_chat_rooms ADD COLUMN access_token VARCHAR(64) DEFAULT NULL"); } catch (\Throwable $e) {}
    try { $db->exec("ALTER TABLE CDS_chat_rooms ADD COLUMN client_email VARCHAR(200) DEFAULT NULL"); } catch (\Throwable $e) {}
    try { $db->exec("ALTER TABLE CDS_chat_rooms ADD COLUMN client_name  VARCHAR(200) DEFAULT NULL"); } catch (\Throwable $e) {}
    try { $db->exec("ALTER TABLE CDS_chat_rooms ADD INDEX idx_token (access_token)"); } catch (\Throwable $e) {}

    $db->exec("CREATE TABLE IF NOT EXISTS CDS_chat_messages (
        id              VARCHAR(32)  NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS CDS_chat_participants (
        room_id      VARCHAR(32)  NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS CDS_chat_pinned (
        id         VARCHAR(32)  NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS CDS_chat_presence (
        user_id  VARCHAR(32) NOT NULL PRIMARY KEY,
        last_seen DATETIME   NOT NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");

    // Colonne topic pour les canaux thématiques
    try { $db->exec("ALTER TABLE CDS_chat_rooms ADD COLUMN topic VARCHAR(300) DEFAULT NULL"); } catch (\Throwable $e) {}

    // S'assurer que cds_users a les colonnes color + profile_picture_url
    // (normalement créées par users.php, mais chat.php peut être appelé avant)
    try { $db->exec("ALTER TABLE cds_users ADD COLUMN color VARCHAR(9) DEFAULT '#c8a96e'"); } catch (\Throwable $e) {}
    try { $db->exec("ALTER TABLE cds_users ADD COLUMN profile_picture_url VARCHAR(400) DEFAULT NULL"); } catch (\Throwable $e) {}
...
// Résout un nom "Prénom Nom" vers l'id de la table cds_users
function chat_resolve_user_by_name($db, $name) {
    $name = trim($name ?? '');
    if ($name === '') return null;
    try {
        // Match exact sur CONCAT(prenom,' ',nom)
        $stmt = $db->prepare("SELECT id, prenom, nom FROM cds_users
        ...
        $stmt = $db->prepare("SELECT id, prenom, nom FROM cds_users WHERE prenom = ? LIMIT 1");
        $stmt->execute([$name]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        return $row ?: null;
    } catch (\Throwable $e) {
...
// Vérifie si cds_users a les colonnes color + profile_picture_url
...
function chat_user_extra_cols() {
    ...
    try {
        $db = getDB();
        $st = $db->query("SHOW COLUMNS FROM cds_users WHERE Field IN ('color','profile_picture_url')");
...
// $projet = row CDS_projets (array). Retourne room_id ou null.
// $createdByName = nom du créateur, $createdById = id du créateur (JWT)
function chat_create_project_room($db, array $projet, $createdByName = null, $createdById = null) {
    chat_ensure_schema();
    if (empty($projet['id'])) return null;

    // Déjà existant ?
    $stmt = $db->prepare("SELECT id FROM CDS_chat_rooms WHERE type='projet' AND projet_id = ? LIMIT 1");
    ...
    $db->prepare("INSERT INTO CDS_chat_rooms (id, type, name, projet_id, created_by)
                  VALUES (?, 'projet', ?, ?, ?)")
    ...
    if ($createdById) {
        ...
        try {
            $stU = $db->prepare("SELECT prenom, nom FROM cds_users WHERE id = ? LIMIT 1");
            $stU->execute([$createdById]);
            $uRow = $stU->fetch(PDO::FETCH_ASSOC);
            if ($uRow) $creatorName = trim(($uRow['prenom'] ?? '') . ' ' . ($uRow['nom'] ?? ''));
        } catch (\Throwable $e) {}
        chat_add_participant_if_missing($db, $roomId, $createdById, $creatorName, null);
    } elseif ($createdByName) {
    ...
    try {
        $stAssignees = $db->prepare("SELECT DISTINCT assignee FROM CDS_taches WHERE projet_id = ? AND assignee IS NOT NULL AND assignee <> ''");
    ...
    } catch (\Throwable $e) { /* silencieux — table CDS_taches peut ne pas exister */ }
...
function chat_hook_project_status($projetId, $statut) {
    ...
    try {
        chat_ensure_schema();
        $db = getDB();
        $stmt = $db->prepare("SELECT id, is_archived FROM CDS_chat_rooms WHERE type='projet' AND projet_id = ? LIMIT 1");
        ...
        if ($shouldArchive && empty($room['is_archived'])) {
            $db->prepare("UPDATE CDS_chat_rooms SET is_archived = 1 WHERE id = ?")->execute([$room['id']]);
            ...
            $db->prepare("UPDATE CDS_chat_rooms SET is_archived = 0 WHERE id = ?")->execute([$room['id']]);
