...
//    curl -s "https://cortobaarchitecture.com/cortobadigitalstudio/api/cron_alerts.php?key=CORTOBA_CRON_2026"
...
function alreadyNotifiedToday(PDO $db, string $userId, string $type, string $linkId): bool {
    try {
        $stmt = $db->prepare("SELECT COUNT(*) FROM CDS_notifications
...
try {
    ...
    $stmt = $db->query("
        ...
        FROM CDS_taches t
        INNER JOIN CDS_tache_livrables l ON l.tache_id = t.id
        LEFT JOIN CDS_projets p ON p.id = t.projet_id
        WHERE t.date_echeance < CURDATE()
          AND t.statut NOT IN ('done','annulée','terminée','Terminée')
        GROUP BY t.id
    ...
    $stmt = $db->query("
        ...
        FROM CDS_projets p
        LEFT JOIN CDS_depenses d ON d.projet_id = p.id
    ...
    $stmt = $db->query("
        ...
        FROM CDS_factures f
        LEFT JOIN CDS_clients c ON c.id = f.client_id
    ...
    if (!empty($holidays)) {
        ...
        $allUsers = $db->query("SELECT id FROM cds_users WHERE statut <> 'Inactif'")->fetchAll(PDO::FETCH_COLUMN);
...
try {
    // Load relance config
    $stmt = $db->prepare("SELECT valeur FROM CDS_parametres WHERE cle = 'relance_config'");
    $stmt->execute();
    ...
    if ($enabled) {
        ...
        $stmt = $db->query("SELECT f.id, f.numero, f.client_id, f.date_echeance, f.relance_niveau, f.derniere_relance,
            ...
            FROM CDS_factures f LEFT JOIN CDS_clients c ON c.id = f.client_id
        ...
        foreach ($overdue as $f) {
            ...
            if ($shouldSend && !empty($f['client_email'])) {
                ...
                // Log
                $db->prepare("INSERT INTO CDS_relances (facture_id, client_id, type, canal, niveau, email_to, statut, cree_par) VALUES (?,?,'auto','email',?,?,?,?)")
                   ->execute([$f['id'], $f['client_id'], $niveau, $f['client_email'], $sent ? 'sent' : 'failed', 'cron']);

                try { $db->exec("ALTER TABLE CDS_factures ADD COLUMN IF NOT EXISTS relance_niveau tinyint DEFAULT 0"); } catch(\Throwable $e){}
                try { $db->exec("ALTER TABLE CDS_factures ADD COLUMN IF NOT EXISTS derniere_relance datetime DEFAULT NULL"); } catch(\Throwable $e){}
...
try {
    ...
    // Load thresholds
    $stmt = $db->prepare("SELECT valeur FROM CDS_parametres WHERE cle = 'alert_thresholds'");
    $stmt->execute();
    ...
    try {
        $stmt = $db->query("SELECT id, code, nom, honoraires_prevus, honoraires_factures, alerte_budget
            FROM CDS_projets WHERE statut IN ('En cours','Actif') AND honoraires_prevus > 0");
        ...
        foreach ($projets as $p) {
            ...
            // Update level
            $db->prepare("UPDATE CDS_projets SET alerte_budget = ? WHERE id = ?")
