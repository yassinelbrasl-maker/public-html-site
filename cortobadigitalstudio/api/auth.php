...
function handleLogin(array $body) {
    ...
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS `cds_users` (
        ...
        $stmt2 = $db->prepare("SELECT * FROM cds_users WHERE email = ? AND is_admin = 0 AND statut != 'Inactif' LIMIT 1");
        $stmt2->execute(array($email));
        $member = $stmt2->fetch(PDO::FETCH_ASSOC);

        if ($member && password_verify($pass, $member['pass_hash'])) {
            // Générer un token et le stocker dans CDS_accounts comme un compte temporaire
    ...
    // ── 2. Vérifier le compte admin (CDS_accounts) ──
    $stmt = $db->prepare('SELECT * FROM CDS_accounts WHERE email = ? LIMIT 1');
    ...
    $db->prepare('UPDATE CDS_accounts SET last_login = NOW() WHERE id = ?')->execute(array($user['id']));
...
function handleMe() {
    ...
    if (!empty($user['isMember'])) {
        try {
            $stmt = $db->prepare('SELECT id, prenom, nom, email, role, modules, profile_picture_url FROM cds_users WHERE id = ? LIMIT 1');
            $stmt->execute(array($user['id']));
        } catch (Exception $e) {
            // Colonne profile_picture_url absente — fallback sans elle
            $stmt = $db->prepare('SELECT id, prenom, nom, email, role, modules FROM cds_users WHERE id = ? LIMIT 1');
            $stmt->execute(array($user['id']));
        }
    ...
    // Compte admin normal
    $stmt = $db->prepare('SELECT id, email, name, role, approved, last_login FROM CDS_accounts WHERE id = ?');
    $stmt->execute(array($user['id']));
    $account = $stmt->fetch();
