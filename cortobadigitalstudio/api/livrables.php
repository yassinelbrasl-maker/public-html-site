...
//  - Catalogue (stocké dans CDS_settings via cds_livrables_catalogue)
//  - Instances par tâche : table CDS_tache_livrables (auto-créée)
//
...
function getList() {
    ...
    $stmt = $db->prepare('SELECT * FROM CDS_tache_livrables WHERE tache_id = ? ORDER BY ordre ASC, cree_at ASC');
...
function createItem(array $body, array $user) {
    ...
    $stmt = $db->prepare('SELECT COALESCE(MAX(ordre),0)+1 AS n FROM CDS_tache_livrables WHERE tache_id = ?');
    $stmt->execute([$tacheId]);
    $ordre = intval(($stmt->fetch() ?: ['n'=>1])['n']);

    $db->prepare('INSERT INTO CDS_tache_livrables (id,tache_id,label,done,catalogue_id,ordre,cree_par) VALUES (?,?,?,?,?,?,?)')
       ->execute([$id, $tacheId, $label, intval(!empty($body['done'])?1:0), $body['catalogue_id'] ?? null, $ordre, $user['name']]);

    $row = $db->prepare('SELECT * FROM CDS_tache_livrables WHERE id = ?');
    $row->execute([$id]);
    jsonOk($row->fetch());
}
...
function updateItem($id, array $user) {
    ...
    $stmt = $db->prepare('SELECT id FROM CDS_tache_livrables WHERE id = ?');
    $stmt->execute([$id]);
    ...
    $params[] = $id;
    $db->prepare('UPDATE CDS_tache_livrables SET ' . implode(', ', $fields) . ' WHERE id = ?')->execute($params);
    ...
    if (array_key_exists('done', $body) && !empty($body['done'])) {
        try {
            $livInfo = $db->prepare('SELECT l.label, l.tache_id, t.titre AS tache_titre, t.assignee, t.projet_id, p.nom AS projet_nom
                FROM CDS_tache_livrables l
                LEFT JOIN CDS_taches t ON t.id = l.tache_id
                LEFT JOIN CDS_projets p ON p.id = t.projet_id
                WHERE l.id = ?');
            $livInfo->execute([$id]);
            $li = $livInfo->fetch();
            if ($li) {
...
function removeItem($id) {
    ...
    $stmt = $db->prepare('SELECT label FROM CDS_tache_livrables WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    $label = $row ? ($row['label'] ?: 'Livrable') : 'Livrable';
    if (!moveToCorbeille($db, 'CDS_tache_livrables', $id, $label, $user['name'] ?? 'unknown')) {
...
// ────────────────────────────────────────────────────────────
//  Préremplissage depuis le catalogue (stocké dans CDS_settings)
//  Matching :
...
function applyCatalogue(array $body, array $user) {
    ...
    $stmt = $db->prepare('SELECT * FROM CDS_taches WHERE id = ?');
    $stmt->execute([$tacheId]);
    $t = $stmt->fetch();
    ...
    if ((int)$t['niveau'] === 0) {
        ...
        if (!empty($t['parent_id'])) {
            $p = $db->prepare('SELECT titre FROM CDS_taches WHERE id = ?');
        ...
        $sousTitre = $t['titre'];
        if (!empty($t['parent_id'])) {
            $p = $db->prepare('SELECT titre, parent_id FROM CDS_taches WHERE id = ?');
            $p->execute([$t['parent_id']]);
            $pr = $p->fetch();
            if ($pr) {
                $tacheTitre = $pr['titre'];
                if (!empty($pr['parent_id'])) {
                    $gp = $db->prepare('SELECT titre FROM CDS_taches WHERE id = ?');
    ...
    $settings = loadSettings(['cds_livrables_catalogue','cds_missions','cds_taches_types']);
    $catalogue    = is_array($settings['cds_livrables_catalogue'] ?? null) ? $settings['cds_livrables_catalogue'] : [];
    $missions     = is_array($settings['cds_missions'] ?? null) ? $settings['cds_missions'] : [];
    $tachesTypes  = is_array($settings['cds_taches_types'] ?? null) ? $settings['cds_taches_types'] : [];
    ...
    $stmt = $db->prepare('SELECT catalogue_id, label FROM CDS_tache_livrables WHERE tache_id = ?');
    $stmt->execute([$tacheId]);
    ...
    $stmtOrd = $db->prepare('SELECT COALESCE(MAX(ordre),0) AS n FROM CDS_tache_livrables WHERE tache_id = ?');
    $stmtOrd->execute([$tacheId]);
    $ordre = intval(($stmtOrd->fetch() ?: ['n'=>0])['n']);

    $ins = $db->prepare('INSERT INTO CDS_tache_livrables (id,tache_id,label,done,catalogue_id,ordre,cree_par) VALUES (?,?,?,0,?,?,?)');
    $added = 0;
...
function loadSettings(array $keys) {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT setting_key, setting_value FROM CDS_settings WHERE setting_key IN ($in)");
        $stmt->execute($keys);
