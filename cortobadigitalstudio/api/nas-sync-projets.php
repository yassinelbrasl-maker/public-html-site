...
try { $db->exec('DELETE FROM CDS_projets_missions'); } catch (\Throwable $e) {}
try { $db->exec('DELETE FROM CDS_projets_intervenants'); } catch (\Throwable $e) {}
$db->exec('DELETE FROM CDS_projets');
...
foreach ($nasFolders as $annee => $folders) {
    ...
    foreach ($folders as $folderName) {
        ...
        if (!$code) {
            ...
            $countStmt = $db->prepare('SELECT COUNT(*) FROM CDS_projets WHERE annee = ?');
        ...
        try {
            $db->prepare('
                INSERT INTO CDS_projets (id, code, nom, client, client_code, annee, phase, statut,
