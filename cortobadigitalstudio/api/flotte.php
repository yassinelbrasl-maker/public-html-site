...
function ensureFlotteTables() {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_vehicules` (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_attributions` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_reservations` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_kilometres` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    // 5. Carburant
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_carburant` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_entretien` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    // 7. Sinistres
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_sinistres` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    // 8. Assurances
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_assurances` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_controles` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_permis` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_flotte_couts` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `vehicule_id` VARCHAR(32) NOT NULL,
...
function listVehicules() {
    $db = getDB();
    $rows = $db->query("SELECT * FROM CDS_flotte_vehicules ORDER BY marque, modele")->fetchAll(PDO::FETCH_ASSOC);
    jsonOk($rows);
}

function getVehicule($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM CDS_flotte_vehicules WHERE id = ?");
    $stmt->execute([$id]);
    $v = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$v) jsonError('Véhicule introuvable', 404);

    // Attribution active
    $s2 = $db->prepare("SELECT * FROM CDS_flotte_attributions WHERE vehicule_id = ? AND statut = 'Active' ORDER BY date_debut DESC LIMIT 1");
    $s2->execute([$id]);
    $v['attribution_active'] = $s2->fetch(PDO::FETCH_ASSOC) ?: null;

    // Assurance active
    $s3 = $db->prepare("SELECT * FROM CDS_flotte_assurances WHERE vehicule_id = ? AND statut = 'Active' ORDER BY date_fin DESC LIMIT 1");
    $s3->execute([$id]);
    $v['assurance_active'] = $s3->fetch(PDO::FETCH_ASSOC) ?: null;

    // Dernier contrôle technique
    $s4 = $db->prepare("SELECT * FROM CDS_flotte_controles WHERE vehicule_id = ? ORDER BY date_controle DESC LIMIT 1");
    $s4->execute([$id]);
    $v['dernier_controle'] = $s4->fetch(PDO::FETCH_ASSOC) ?: null;

    // Prochain entretien planifié
    $s5 = $db->prepare("SELECT * FROM CDS_flotte_entretien WHERE vehicule_id = ? AND statut = 'Planifié' ORDER BY date_prevue ASC LIMIT 1");
    $s5->execute([$id]);
...
function createVehicule($user) {
    ...
    $db->prepare("INSERT INTO CDS_flotte_vehicules (id, marque, modele, immatriculation, vin, type_vehicule, couleur,
...
function updateVehicule($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_vehicules SET
...
function deleteVehicule($id) {
    ...
    $db = getDB();
    $stmt = $db->prepare("SELECT immatriculation, marque, modele FROM CDS_flotte_vehicules WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['immatriculation'] ?? '') . ' ' . ($row['marque'] ?? '') . ' ' . ($row['modele'] ?? '')) : 'Véhicule';
    if (!moveToCorbeille($db, 'CDS_flotte_vehicules', $id, $label, $user['name'] ?? 'unknown')) {
...
function listAttributions() {
    ...
    if ($vid) {
        $stmt = $db->prepare("SELECT a.*, v.marque, v.modele, v.immatriculation
                              FROM CDS_flotte_attributions a
                              LEFT JOIN CDS_flotte_vehicules v ON v.id = a.vehicule_id
...
function createAttribution($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_attributions (id, vehicule_id, type_attribution, collaborateur, projet_id,
    ...
    $db->prepare("UPDATE CDS_flotte_vehicules SET statut = 'Attribué' WHERE id = ?")->execute([$b['vehicule_id'] ?? '']);
    jsonOk(['id' => $id]);
}

function updateAttribution($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_attributions SET
    ...
    if (($b['statut'] ?? '') === 'Terminée') {
        $stmt = $db->prepare("SELECT vehicule_id FROM CDS_flotte_attributions WHERE id = ?");
        $stmt->execute([$id]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $db->prepare("UPDATE CDS_flotte_vehicules SET statut = 'Disponible' WHERE id = ?")->execute([$row['vehicule_id']]);
        }
    }
    jsonOk(['updated' => true]);
}
...
function deleteAttribution($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT collaborateur, type_attribution FROM CDS_flotte_attributions WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['type_attribution'] ?? '') . ' — ' . ($row['collaborateur'] ?? '')) : 'Attribution';
    if (!moveToCorbeille($db, 'CDS_flotte_attributions', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function listReservations() {
    ...
    if ($vid) {
        $stmt = $db->prepare("SELECT r.*, v.marque, v.modele, v.immatriculation
                              FROM CDS_flotte_reservations r
                              LEFT JOIN CDS_flotte_vehicules v ON v.id = r.vehicule_id
...
function createReservation($user) {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) FROM CDS_flotte_reservations
    ...
    $db->prepare("INSERT INTO CDS_flotte_reservations (id, vehicule_id, demandeur, date_debut, date_fin,
...
function updateReservation($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_reservations SET statut=?, approuve_par=? WHERE id=?")
...
function deleteReservation($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT demandeur, destination FROM CDS_flotte_reservations WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['demandeur'] ?? '') . ' — ' . ($row['destination'] ?? '')) : 'Réservation';
    if (!moveToCorbeille($db, 'CDS_flotte_reservations', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function listKilometres() {
    ...
    if ($vid) {
        $stmt = $db->prepare("SELECT k.*, v.marque, v.modele, v.immatriculation
                              FROM CDS_flotte_kilometres k
                              LEFT JOIN CDS_flotte_vehicules v ON v.id = k.vehicule_id
...
function createKilometrage($user) {
    ...
    $db->prepare("INSERT INTO CDS_flotte_kilometres (id, vehicule_id, date_releve, km_debut, km_fin, distance,
    ...
    if (($b['km_fin'] ?? 0) > 0) {
        $db->prepare("UPDATE CDS_flotte_vehicules SET kilometrage_actuel = GREATEST(kilometrage_actuel, ?) WHERE id = ?")
...
function deleteKilometrage($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT date_releve, destination FROM CDS_flotte_kilometres WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['date_releve'] ?? '') . ' ' . ($row['destination'] ?? '')) : 'Kilométrage';
    if (!moveToCorbeille($db, 'CDS_flotte_kilometres', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function listCarburant() {
    ...
    if ($vid) {
        $stmt = $db->prepare("SELECT c.*, v.marque, v.modele, v.immatriculation
                              FROM CDS_flotte_carburant c
                              LEFT JOIN CDS_flotte_vehicules v ON v.id = c.vehicule_id
...
function createCarburant($user) {
    ...
    $db->prepare("INSERT INTO CDS_flotte_carburant (id, vehicule_id, date_plein, km_compteur, litres, prix_litre,
...
function deleteCarburant($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT date_plein, station FROM CDS_flotte_carburant WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['date_plein'] ?? '') . ' ' . ($row['station'] ?? '')) : 'Carburant';
    if (!moveToCorbeille($db, 'CDS_flotte_carburant', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function listEntretien() {
    ...
    if ($vid) {
        $stmt = $db->prepare("SELECT e.*, v.marque, v.modele, v.immatriculation
                              FROM CDS_flotte_entretien e
                              LEFT JOIN CDS_flotte_vehicules v ON v.id = e.vehicule_id
...
function createEntretien($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_entretien (id, vehicule_id, type_entretien, categorie, titre, description,
...
function updateEntretien($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_entretien SET
    ...
    if (($b['statut'] ?? '') === 'En cours') {
        $db->prepare("UPDATE CDS_flotte_vehicules SET statut = 'En réparation' WHERE id = ?")->execute([$b['vehicule_id'] ?? '']);
    } elseif (($b['statut'] ?? '') === 'Terminé') {
        $db->prepare("UPDATE CDS_flotte_vehicules SET statut = 'Disponible' WHERE id = ?")->execute([$b['vehicule_id'] ?? '']);
    }
    jsonOk(['updated' => true]);
}
...
function deleteEntretien($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT titre FROM CDS_flotte_entretien WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? ($row['titre'] ?: 'Entretien') : 'Entretien';
    if (!moveToCorbeille($db, 'CDS_flotte_entretien', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function listSinistres() {
    ...
    if ($vid) {
        $stmt = $db->prepare("SELECT s.*, v.marque, v.modele, v.immatriculation
                              FROM CDS_flotte_sinistres s
                              LEFT JOIN CDS_flotte_vehicules v ON v.id = s.vehicule_id
...
function createSinistre($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_sinistres (id, vehicule_id, date_sinistre, type_sinistre, lieu,
...
function updateSinistre($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_sinistres SET
...
function deleteSinistre($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT type_sinistre, date_sinistre FROM CDS_flotte_sinistres WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['type_sinistre'] ?? '') . ' ' . ($row['date_sinistre'] ?? '')) : 'Sinistre';
    if (!moveToCorbeille($db, 'CDS_flotte_sinistres', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function createAssurance($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_assurances (id, vehicule_id, assureur, numero_police, type_couverture,
...
function updateAssurance($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_assurances SET
...
function deleteAssurance($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT assureur, numero_police FROM CDS_flotte_assurances WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['assureur'] ?? '') . ' ' . ($row['numero_police'] ?? '')) : 'Assurance';
    if (!moveToCorbeille($db, 'CDS_flotte_assurances', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function createControle($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_controles (id, vehicule_id, type_controle, date_controle, date_expiration,
...
function deleteControle($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT type_controle, date_controle FROM CDS_flotte_controles WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['type_controle'] ?? '') . ' ' . ($row['date_controle'] ?? '')) : 'Contrôle';
    if (!moveToCorbeille($db, 'CDS_flotte_controles', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function listPermis() {
    $db = getDB();
    $rows = $db->query("SELECT * FROM CDS_flotte_permis ORDER BY collaborateur")->fetchAll(PDO::FETCH_ASSOC);
    jsonOk($rows);
}
...
function createPermis($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_permis (id, collaborateur, numero_permis, categorie, date_delivrance,
...
function updatePermis($id) {
    $b = getCleanBody(); $db = getDB();
    $db->prepare("UPDATE CDS_flotte_permis SET
...
function deletePermis($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT collaborateur, numero_permis FROM CDS_flotte_permis WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['collaborateur'] ?? '') . ' ' . ($row['numero_permis'] ?? '')) : 'Permis';
    if (!moveToCorbeille($db, 'CDS_flotte_permis', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function createCout($user) {
    $b = getCleanBody(); $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_flotte_couts (id, vehicule_id, categorie, libelle, date_cout, montant, notes)
                  VALUES (?,?,?,?,?,?,?)")
       ->execute([
...
function deleteCout($id) {
    ...
    if (($user['role'] ?? '') !== 'admin') jsonError('Admin requis', 403);
    $db = getDB();
    $stmt = $db->prepare("SELECT libelle, categorie FROM CDS_flotte_couts WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $label = $row ? trim(($row['categorie'] ?? '') . ' — ' . ($row['libelle'] ?? '')) : 'Coût';
    if (!moveToCorbeille($db, 'CDS_flotte_couts', $id, $label, $user['name'] ?? 'unknown')) {
        jsonError('Impossible de déplacer vers la corbeille', 500);
    }
    jsonOk(['deleted' => true]);
}
...
function getFlotteDashboard() {
    ...
    $total = (int)$db->query("SELECT COUNT(*) FROM CDS_flotte_vehicules")->fetchColumn();
    $disponibles = (int)$db->query("SELECT COUNT(*) FROM CDS_flotte_vehicules WHERE statut = 'Disponible'")->fetchColumn();
    $attribues = (int)$db->query("SELECT COUNT(*) FROM CDS_flotte_vehicules WHERE statut = 'Attribué'")->fetchColumn();
    $enReparation = (int)$db->query("SELECT COUNT(*) FROM CDS_flotte_vehicules WHERE statut = 'En réparation'")->fetchColumn();
    $horsService = (int)$db->query("SELECT COUNT(*) FROM CDS_flotte_vehicules WHERE statut = 'Hors service'")->fetchColumn();

    // Coûts du mois
    $mois = date('Y-m');
    $carburantMois = (float)$db->query("SELECT COALESCE(SUM(montant_total),0) FROM CDS_flotte_carburant WHERE date_plein LIKE '$mois%'")->fetchColumn();
    $entretienMois = (float)$db->query("SELECT COALESCE(SUM(montant),0) FROM CDS_flotte_entretien WHERE (date_realisee LIKE '$mois%' OR date_prevue LIKE '$mois%') AND statut != 'Annulé'")->fetchColumn();
    $coutsMois = (float)$db->query("SELECT COALESCE(SUM(montant),0) FROM CDS_flotte_couts WHERE date_cout LIKE '$mois%'")->fetchColumn();

    // KM total du mois
    $kmMois = (int)$db->query("SELECT COALESCE(SUM(distance),0) FROM CDS_flotte_kilometres WHERE date_releve LIKE '$mois%'")->fetchColumn();

    // Réservations en attente
    $resaEnAttente = (int)$db->query("SELECT COUNT(*) FROM CDS_flotte_reservations WHERE statut = 'En attente'")->fetchColumn();
...
function getTCO() {
    ...
    $sql = "SELECT v.id, v.marque, v.modele, v.immatriculation, v.valeur_achat, v.date_achat, v.kilometrage_actuel,
            ...
            FROM CDS_flotte_vehicules v
            LEFT JOIN (SELECT vehicule_id, SUM(montant_total) AS total FROM CDS_flotte_carburant GROUP BY vehicule_id) carb ON carb.vehicule_id = v.id
            LEFT JOIN (SELECT vehicule_id, SUM(montant) AS total FROM CDS_flotte_entretien WHERE statut != 'Annulé' GROUP BY vehicule_id) ent ON ent.vehicule_id = v.id
            LEFT JOIN (SELECT vehicule_id, SUM(prime_annuelle) AS total FROM CDS_flotte_assurances GROUP BY vehicule_id) ass ON ass.vehicule_id = v.id
            LEFT JOIN (SELECT vehicule_id, SUM(montant) AS total FROM CDS_flotte_couts GROUP BY vehicule_id) ct ON ct.vehicule_id = v.id
            LEFT JOIN (SELECT vehicule_id, SUM(montant_degats) - SUM(montant_rembourse) AS total FROM CDS_flotte_sinistres GROUP BY vehicule_id) sin ON sin.vehicule_id = v.id
            $where
...
function getAlertes() {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_flotte_permis WHERE date_expiration BETWEEN ? AND ?");
    $stmt->execute([$today, $dans30j]);
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
