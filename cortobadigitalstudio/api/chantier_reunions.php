...
function ensureReunionsTables() {
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_reunions` (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_reunion_actions` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
...
function listReunions() {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_chantier_reunions WHERE chantier_id=? ORDER BY date_reunion DESC");
    $stmt->execute([$cid]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    foreach ($rows as &$r) {
        $r['participants'] = json_decode($r['participants'] ?: '[]', true);
        $s2 = $db->prepare("SELECT * FROM CDS_chantier_reunion_actions WHERE reunion_id=? ORDER BY cree_at");
        $s2->execute([$r['id']]);
...
function getReunion($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM CDS_chantier_reunions WHERE id=?");
    $stmt->execute([$id]);
    ...
    $s3 = $db->prepare("SELECT a.*, r.numero AS reunion_numero FROM CDS_chantier_reunion_actions a
                         JOIN CDS_chantier_reunions r ON r.id = a.reunion_id
...
function createReunion($user) {
    ...
    $cid = $b['chantier_id'] ?? '';
    // Auto-numérotation
    $s = $db->prepare("SELECT COALESCE(MAX(numero),0)+1 AS next_num FROM CDS_chantier_reunions WHERE chantier_id=?");
    $s->execute([$cid]);
    $num = $s->fetch(PDO::FETCH_ASSOC)['next_num'];

    $db->prepare("INSERT INTO CDS_chantier_reunions (id, chantier_id, numero, date_reunion, lieu, objet, participants, points_discutes, decisions, pv_contenu, statut, cree_par)
                  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
    ...
    if (!empty($b['actions']) && is_array($b['actions'])) {
        foreach ($b['actions'] as $act) {
            ...
            $db->prepare("INSERT INTO CDS_chantier_reunion_actions (id, reunion_id, chantier_id, description, responsable, delai, statut)
                          VALUES (?,?,?,?,?,?,?)")
...
function updateReunion($id, $user) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_reunions SET date_reunion=?, lieu=?, objet=?,
...
function deleteReunion($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_reunion_actions WHERE reunion_id=?")->execute([$id]);
    $db->prepare("DELETE FROM CDS_chantier_reunions WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}

function diffuserPV($id, $user) {
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_reunions SET statut='Diffusé', diffuse_at=NOW() WHERE id=?")
       ->execute([$id]);
...
function listActions() {
    ...
    $sql = "SELECT a.*, r.numero AS reunion_numero, r.date_reunion
            FROM CDS_chantier_reunion_actions a
            JOIN CDS_chantier_reunions r ON r.id = a.reunion_id
            WHERE a.chantier_id=?";
    $params = [$cid];
...
function updateAction($id) {
    ...
    $params[] = $id;
    $db->prepare("UPDATE CDS_chantier_reunion_actions SET $sets WHERE id=?")->execute($params);
    jsonOk(['updated' => true]);
}

function deleteAction($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_reunion_actions WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
