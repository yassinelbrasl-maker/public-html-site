...
try {
    $db = getDB();
    $db->prepare("REPLACE INTO CDS_chat_presence (user_id, last_seen) VALUES (?, NOW())")
       ->execute([$user['id'] ?? '']);
} catch (\Throwable $e) {}
...
function canAccessRoom($db, $user, $room) {
    ...
    // Participant direct
    $stmt = $db->prepare("SELECT 1 FROM CDS_chat_participants WHERE room_id = ? AND user_id = ?");
...
function listRooms($user) {
    ...
    $projetInfos = [];
    try {
        $stP = $db->query("SELECT id, nom, code FROM CDS_projets");
    ...
    foreach ($mine as &$r) {
        ...
        if ($r['type'] === 'direct') {
            try {
                $st2 = $db->prepare("SELECT cp.user_id, cp.user_name, u.profile_picture_url, u.color, u.role
                                     FROM CDS_chat_participants cp
                                     LEFT JOIN cds_users u ON u.id = cp.user_id
                                     WHERE cp.room_id = ? AND cp.user_id <> ? LIMIT 1");
                $st2->execute([$r['id'], $uid]);
                $other = $st2->fetch(PDO::FETCH_ASSOC);
            } catch (\Throwable $e) {
                $st2 = $db->prepare("SELECT cp.user_id, cp.user_name FROM CDS_chat_participants cp WHERE cp.room_id = ? AND cp.user_id <> ? LIMIT 1");
                $st2->execute([$r['id'], $uid]);
                $other = $st2->fetch(PDO::FETCH_ASSOC);
        ...
        if ($r['type'] === 'projet' || $r['type'] === 'canal') {
            try {
                ...
                $st3 = $db->prepare("SELECT cp.user_id, cp.user_name FROM CDS_chat_participants cp WHERE cp.room_id = ? ORDER BY cp.joined_at ASC");
                $st3->execute([$r['id']]);
                $r['participants'] = $st3->fetchAll(PDO::FETCH_ASSOC);
            }
        }
    ...
    if (chat_is_privileged($user)) {
        ...
        $sql2 = "SELECT r.*, 0 AS is_favorite, NULL AS last_read_at,
                        0 AS unread,
                        (SELECT m2.content FROM CDS_chat_messages m2 WHERE m2.room_id = r.id ORDER BY m2.cree_at DESC LIMIT 1) AS last_message,
                        (SELECT m2.cree_at FROM CDS_chat_messages m2 WHERE m2.room_id = r.id ORDER BY m2.cree_at DESC LIMIT 1) AS last_message_at
                 FROM CDS_chat_rooms r
                 WHERE r.type = 'projet' "
    ...
    foreach ($rooms as &$r) {
        if ($r['type'] === 'canal') {
            try {
                $stT = $db->prepare("SELECT topic FROM CDS_chat_rooms WHERE id = ?");
                $stT->execute([$r['id']]);
            ...
            // Participants count
            try {
                $stC = $db->prepare("SELECT COUNT(*) FROM CDS_chat_participants WHERE room_id = ?");
                $stC->execute([$r['id']]);
...
function listMessages($user) {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_chat_rooms WHERE id = ?");
    $stmt->execute([$roomId]);
    ...
    try {
        $stmt = $db->prepare("SELECT m.*, u.color, u.profile_picture_url
                              FROM CDS_chat_messages m
                              LEFT JOIN cds_users u ON u.id = m.sender_id
                              WHERE $where ORDER BY m.cree_at ASC LIMIT 500");
        $stmt->execute($params);
    } catch (\Throwable $e) {
        $stmt = $db->prepare("SELECT m.* FROM CDS_chat_messages m WHERE $where ORDER BY m.cree_at ASC LIMIT 500");
        $stmt->execute($params);
    }
...
function sendMessage($user) {
    ...
    $db->prepare("INSERT INTO CDS_chat_messages (id, room_id, sender_id, sender_name, kind, content, attachment_url, attachment_name)
                  VALUES (?,?,?,?,?,?,?,?)")
       ->execute([$mid, $roomId, $user['id'] ?? '', $user['name'] ?? '', $kind, $content, $attUrl ?: null, $attName ?: null]);

    // Marquer la room comme lue pour l'expéditeur
    $db->prepare("UPDATE CDS_chat_participants SET last_read_at = NOW()
    ...
    try {
        $stPart = $db->prepare("SELECT user_id FROM CDS_chat_participants WHERE room_id = ? AND user_id <> ?");
        ...
        foreach ($recipients as $rid) {
            ...
            $existing = null;
            try {
                $stEx = $db->prepare("SELECT id FROM CDS_notifications WHERE user_id = ? AND type IN ('chat','chat_mention') AND link_id = ? AND is_read = 0 ORDER BY cree_at DESC LIMIT 1");
            ...
            if ($existing) {
                ...
                try {
                    $db->prepare("UPDATE CDS_notifications SET title = ?, message = ?, type = ?, cree_par = ?, cree_at = NOW() WHERE id = ?")
    ...
    try {
        $stmt = $db->prepare("SELECT m.*, u.color, u.profile_picture_url FROM CDS_chat_messages m LEFT JOIN cds_users u ON u.id = m.sender_id WHERE m.id = ?");
        $stmt->execute([$mid]);
    } catch (\Throwable $e) {
        $stmt = $db->prepare("SELECT m.* FROM CDS_chat_messages m WHERE m.id = ?");
        $stmt->execute([$mid]);
    }
...
function openDirect($user) {
    ...
    $sql = "SELECT r.id FROM CDS_chat_rooms r
            INNER JOIN CDS_chat_participants p1 ON p1.room_id = r.id AND p1.user_id = ?
            INNER JOIN CDS_chat_participants p2 ON p2.room_id = r.id AND p2.user_id = ?
    ...
    $otherRow = null;
    try {
        $st = $db->prepare("SELECT id, prenom, nom FROM cds_users WHERE id = ? LIMIT 1");
        $st->execute([$other]);
    ...
    $roomId = chat_genid();
    $db->prepare("INSERT INTO CDS_chat_rooms (id, type, name, created_by) VALUES (?, 'direct', NULL, ?)")
       ->execute([$roomId, $user['name'] ?? '']);

    $db->prepare("INSERT INTO CDS_chat_participants (room_id, user_id, user_name) VALUES (?,?,?)")
...
function addParticipant($user) {
    ...
    $st = $db->prepare("SELECT id, prenom, nom FROM cds_users WHERE id = ?");
    $st->execute([$target]);
...
function removeParticipant($user) {
    ...
    $targetName = '';
    try {
        $st = $db->prepare("SELECT user_name FROM CDS_chat_participants WHERE room_id = ? AND user_id = ?");
        $st->execute([$roomId, $target]);
        $row = $st->fetch(PDO::FETCH_ASSOC);
        $targetName = $row['user_name'] ?? '';
    } catch (\Throwable $e) {}

    $db->prepare("DELETE FROM CDS_chat_participants WHERE room_id = ? AND user_id = ?")
...
function unreadCount($user) {
    ...
    $sql = "SELECT COALESCE(SUM(x.n),0) AS total FROM (
              SELECT COUNT(*) AS n
              FROM CDS_chat_messages m
              INNER JOIN CDS_chat_participants p ON p.room_id = m.room_id AND p.user_id = ?
...
function listUsers($user) {
    ...
    try {
        $stmt2 = $db->query("SELECT user_id FROM CDS_chat_presence WHERE last_seen > (NOW() - INTERVAL 90 SECOND)");
...
function listProjectsForChat($user) {
    ...
    try {
        ...
        $sql = "SELECT p.id, p.nom, p.code, p.statut
                FROM CDS_projets p
                WHERE NOT EXISTS (
                    SELECT 1 FROM CDS_chat_rooms r WHERE r.type='projet' AND r.projet_id = p.id
                )
        ...
        try {
            $sql2 = "SELECT p.id, p.nom, p.code FROM CDS_projets p
                     WHERE NOT EXISTS (SELECT 1 FROM CDS_chat_rooms r WHERE r.type='projet' AND r.projet_id = p.id)
                     ORDER BY p.code ASC, p.nom ASC";
            $stmt = $db->query($sql2);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (\Throwable $e2) {
...
function createCanal($user) {
    ...
    $roomId = chat_genid();
    $db->prepare("INSERT INTO CDS_chat_rooms (id, type, name, topic, created_by) VALUES (?, 'canal', ?, ?, ?)")
...
function joinCanal($user) {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_chat_rooms WHERE id = ? AND type = 'canal'");
    $stmt->execute([$roomId]);
    $room = $stmt->fetch(PDO::FETCH_ASSOC);
...
function listCanaux($user) {
    ...
    try {
        $stmt = $db->query("SELECT r.id, r.name, r.topic, r.is_archived,
                                   (SELECT COUNT(*) FROM CDS_chat_participants WHERE room_id = r.id) AS member_count
                            FROM CDS_chat_rooms r
                            WHERE r.type = 'canal'
                            ORDER BY r.name ASC");
    } catch (\Throwable $e) {
    ...
    $uid = $user['id'] ?? '';
    foreach ($rows as &$r) {
        $st = $db->prepare("SELECT 1 FROM CDS_chat_participants WHERE room_id = ? AND user_id = ?");
        $st->execute([$r['id'], $uid]);
...
function pinMessage($user) {
    ...
    $stmt = $db->prepare("SELECT m.*, r.type, r.projet_id FROM CDS_chat_messages m
                          INNER JOIN CDS_chat_rooms r ON r.id = m.room_id
                          WHERE m.id = ?");
    $stmt->execute([$mid]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $id = chat_genid();
    $db->prepare("INSERT INTO CDS_chat_pinned (id, message_id, projet_id, pinned_by, label) VALUES (?,?,?,?,?)")
...
function createClientPortal($user) {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_chat_rooms WHERE type='client' AND projet_id = ? LIMIT 1");
    $stmt->execute([$pid]);
    ...
    if ($existing) {
        ...
        if ($cname || $cmail) {
            $db->prepare("UPDATE CDS_chat_rooms SET client_name = COALESCE(NULLIF(?,''), client_name),
        ...
        $db->prepare("INSERT INTO CDS_chat_rooms (id, type, name, projet_id, created_by, access_token, client_name, client_email)
                      VALUES (?, 'client', ?, ?, ?, ?, ?, ?)")
    ...
    $dir    = rtrim(dirname(dirname($_SERVER['SCRIPT_NAME'] ?? '/')), '/'); // .../cortobadigitalstudio
...
function clientView() {
    ...
    $stmt = $db->prepare("SELECT r.*, p.nom AS projet_nom, p.code AS projet_code, p.phase AS projet_phase
                          FROM CDS_chat_rooms r
                          LEFT JOIN CDS_projets p ON p.id COLLATE utf8mb4_unicode_ci = r.projet_id COLLATE utf8mb4_unicode_ci
    ...
    $stmt = $db->prepare("SELECT id, sender_id, sender_name, kind, content, cree_at
                          FROM CDS_chat_messages WHERE room_id = ? ORDER BY cree_at ASC LIMIT 500");
...
function clientSend() {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_chat_rooms WHERE type='client' AND access_token = ? LIMIT 1");
    $stmt->execute([$token]);
    $room = $stmt->fetch(PDO::FETCH_ASSOC);
...
function listPinned($user) {
    ...
    $stmt = $db->prepare("SELECT pin.*, m.content, m.sender_name, m.cree_at AS message_at
                          FROM CDS_chat_pinned pin
                          INNER JOIN CDS_chat_messages m ON m.id = pin.message_id
