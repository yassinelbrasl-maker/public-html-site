...
try {
    ...
    $_db0->exec("CREATE TABLE IF NOT EXISTS `CDS_notification_prefs` (
    ...
    $_db0->exec("CREATE TABLE IF NOT EXISTS `CDS_push_subscriptions` (
...
define('VAPID_PUBLIC_KEY',  'BOxO-y8bMDB9jEndAQ71pNG4r5wTie7dp_WS51BG_IOYyXFan-0HwcRj3L4nOIfIztbHZgsGNd4EnbF4Fj5GQa0');
define('VAPID_PRIVATE_KEY', 'MtpbwbhSPOVWGyUxAcC6c1Z4Bo6ikZEc-qb-FBEhPkc');
...
try {
    switch ($action) {
        ...
        case 'get': {
            ...
            $stmt = $db->prepare("SELECT * FROM CDS_notification_prefs WHERE user_id = ?");
            ...
            $pushStmt = $db->prepare("SELECT COUNT(*) FROM CDS_push_subscriptions WHERE user_id = ?");
            ...
            $userEmail = '';
            try {
                $emailStmt = $db->prepare("SELECT email_pro, email_perso, email FROM cds_users WHERE id = ?");
                ...
                try {
                    $emailStmt2 = $db->prepare("SELECT email FROM cds_users WHERE id = ?");
                    $emailStmt2->execute([$user['id']]);
                    $emailRow2 = $emailStmt2->fetch();
                    if ($emailRow2) $userEmail = $emailRow2['email'] ?? '';
                } catch (\Throwable $e2) {}
            }
            // Fallback : chercher dans CDS_accounts (pour les admins)
            if (!$userEmail) {
                try {
                    $emailStmt3 = $db->prepare("SELECT email FROM CDS_accounts WHERE id = ?");
        ...
        case 'save': {
            ...
            $stmt = $db->prepare("REPLACE INTO CDS_notification_prefs
        ...
        case 'push_subscribe': {
            ...
            $db->prepare("DELETE FROM CDS_push_subscriptions WHERE endpoint = ?")
               ->execute([$endpoint]);

            $id = bin2hex(random_bytes(16));
            $db->prepare("INSERT INTO CDS_push_subscriptions (id, user_id, endpoint, p256dh, auth, user_agent)
                          VALUES (?, ?, ?, ?, ?, ?)")
        ...
        case 'push_unsubscribe': {
            ...
            $endpoint = $body['endpoint'] ?? '';
            if ($endpoint) {
                $db->prepare("DELETE FROM CDS_push_subscriptions WHERE user_id = ? AND endpoint = ?")
                   ->execute([$user['id'], $endpoint]);
            } else {
                // Supprimer tous les abonnements de l'utilisateur
                $db->prepare("DELETE FROM CDS_push_subscriptions WHERE user_id = ?")
                   ->execute([$user['id']]);
            }
            jsonOk(['ok' => true]);
            break;
