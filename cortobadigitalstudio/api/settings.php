...
// ════════════════════════════════════════════════════════
if ($method === 'GET') {
    $rows = $db->query("SELECT setting_key, setting_value FROM CDS_settings")->fetchAll();
    $data = [];
    foreach ($rows as $row) {
...
if ($method === 'POST') {
    ...
    $stmt = $db->prepare("
        INSERT INTO CDS_settings (setting_key, setting_value)
        VALUES (?, ?)
...
if ($method === 'DELETE') {
    ...
    $stmt = $db->prepare("DELETE FROM CDS_settings WHERE setting_key = :k");
    $stmt->execute([':k' => $key]);
