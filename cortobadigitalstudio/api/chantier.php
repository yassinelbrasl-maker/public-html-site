...
function ensureChantierTables() {
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantiers` (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_lots` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_taches` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_tache_deps` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_journal` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_phases` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    // Migrate CDS_chantier_phases — add lot_id, chantier_id, avancement columns if missing
    try {
        $cols = [];
        $colStmt = $db->query("SHOW COLUMNS FROM CDS_chantier_phases");
        foreach ($colStmt->fetchAll(PDO::FETCH_ASSOC) as $c) { $cols[] = $c['Field']; }
        if (!in_array('lot_id', $cols)) {
            try { $db->exec("ALTER TABLE CDS_chantier_phases ADD COLUMN `lot_id` VARCHAR(32) DEFAULT NULL AFTER `id`"); } catch (Exception $e) {}
            try { $db->exec("ALTER TABLE CDS_chantier_phases ADD KEY `idx_phase_lot` (`lot_id`)"); } catch (Exception $e) {}
        }
        if (!in_array('chantier_id', $cols)) {
            try { $db->exec("ALTER TABLE CDS_chantier_phases ADD COLUMN `chantier_id` VARCHAR(32) DEFAULT NULL AFTER `lot_id`"); } catch (Exception $e) {}
            try { $db->exec("ALTER TABLE CDS_chantier_phases ADD KEY `idx_phase_chantier` (`chantier_id`)"); } catch (Exception $e) {}
        }
        if (!in_array('avancement', $cols)) {
            try { $db->exec("ALTER TABLE CDS_chantier_phases ADD COLUMN `avancement` INT NOT NULL DEFAULT 0 AFTER `actif`"); } catch (Exception $e) {}
        }
    } catch (Exception $e) {}

    // Seed default phases if table empty
    try {
        $cnt = $db->query("SELECT COUNT(*) FROM CDS_chantier_phases")->fetchColumn();
        if ($cnt == 0) {
            ...
            foreach ($phases as $ph) {
                ...
                $db->prepare("INSERT INTO CDS_chantier_phases (id, nom, ordre) VALUES (?,?,?)")->execute([$pid, $ph, $ord++]);
            }
        }
    } catch (Exception $e) {}
    ...
    try {
        $cols = [];
        $colStmt = $db->query("SHOW COLUMNS FROM CDS_chantier_journal");
        foreach ($colStmt->fetchAll(PDO::FETCH_ASSOC) as $c) { $cols[] = $c['Field']; }
        $migrations = [
        ...
        foreach ($migrations as $m) {
            if (!in_array($m[0], $cols)) {
                try { $db->exec("ALTER TABLE CDS_chantier_journal " . $m[1]); } catch (Exception $e) {}
            }
        }
    } catch (Exception $e) {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_effectifs` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_intervenants` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_param_lots` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `nom` VARCHAR(200) NOT NULL,
      `code` VARCHAR(20) DEFAULT NULL,
      `ordre` INT NOT NULL DEFAULT 0,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_param_lot_phases` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CDS_chantier_lot_phases` (
      `id` VARCHAR(32) NOT NULL PRIMARY KEY,
      `chantier_id` VARCHAR(32) NOT NULL,
    ...
    // Migrate CDS_chantiers — add lot_depart, lot_fin columns if missing
    try {
        $cols = [];
        $colStmt = $db->query("SHOW COLUMNS FROM CDS_chantiers");
        foreach ($colStmt->fetchAll(PDO::FETCH_ASSOC) as $c) { $cols[] = $c['Field']; }
        if (!in_array('lot_depart', $cols)) {
            try { $db->exec("ALTER TABLE CDS_chantiers ADD COLUMN `lot_depart` VARCHAR(200) DEFAULT NULL AFTER `description`"); } catch (Exception $e) {}
        }
        if (!in_array('lot_fin', $cols)) {
            try { $db->exec("ALTER TABLE CDS_chantiers ADD COLUMN `lot_fin` VARCHAR(200) DEFAULT NULL AFTER `lot_depart`"); } catch (Exception $e) {}
        }
    } catch (Exception $e) {}

    // Seed default param lots if table empty
    try {
        $cnt = $db->query("SELECT COUNT(*) FROM CDS_param_lots")->fetchColumn();
        if ($cnt == 0) {
            $lotsWithPhases = [
            ...
            $ord = 1;
            foreach ($lotsWithPhases as $ln => $phases) {
                $lid = bin2hex(random_bytes(16));
                $db->prepare("INSERT INTO CDS_param_lots (id, nom, ordre) VALUES (?,?,?)")->execute([$lid, $ln, $ord++]);
                $phaseOrd = 1;
                foreach ($phases as $phaseName) {
                    $pid = bin2hex(random_bytes(16));
                    $db->prepare("INSERT INTO CDS_param_lot_phases (id, param_lot_id, nom, ordre) VALUES (?,?,?,?)")->execute([$pid, $lid, $phaseName, $phaseOrd++]);
                }
            }
        }
    } catch (Exception $e) {}
...
try {
    ...
    $check = $db->query("SELECT COUNT(*) FROM CDS_param_lot_phases WHERE nom='Implantation topographique'")->fetchColumn();
    if ($check == 0) {
        $lotsPhasesMigration = [
            'Terrassement' => ['Implantation topographique','Decapage terre vegetale','Excavation/Deblais','Remblaiement','Compactage','Evacuation des terres'],
            'Fondations' => ['Etude geotechnique','Fouilles','Beton de proprete','Ferraillage','Coulage beton','Remblaiement peripherique'],
        ...
        $maxOrd = (int)$db->query("SELECT COALESCE(MAX(ordre),0) FROM CDS_param_lots")->fetchColumn();
        foreach ($lotsPhasesMigration as $lotName => $phases) {
            // Check if lot already exists
            $stmt = $db->prepare("SELECT id FROM CDS_param_lots WHERE nom=?");
            $stmt->execute([$lotName]);
            $lotId = $stmt->fetchColumn();
            if ($lotId) {
                // Existing lot: delete old generic phases, add new ones
                $db->prepare("DELETE FROM CDS_param_lot_phases WHERE param_lot_id=?")->execute([$lotId]);
            } else {
                // New lot: insert it
                $lotId = bin2hex(random_bytes(16));
                $maxOrd++;
                $db->prepare("INSERT INTO CDS_param_lots (id, nom, ordre) VALUES (?,?,?)")->execute([$lotId, $lotName, $maxOrd]);
            }
            // Insert detailed phases
            $phaseOrd = 1;
            foreach ($phases as $phaseName) {
                $pid = bin2hex(random_bytes(16));
                $db->prepare("INSERT INTO CDS_param_lot_phases (id, param_lot_id, nom, ordre) VALUES (?,?,?,?)")->execute([$pid, $lotId, $phaseName, $phaseOrd++]);
            }
        }
    }
...
function listChantiers() {
    ...
    $sql = "SELECT c.*, p.nom AS projet_nom, p.code AS projet_code
            FROM CDS_chantiers c
            LEFT JOIN CDS_projets p ON p.id = c.projet_id
...
function getChantier($id) {
    ...
    // Lots
    $s2 = $db->prepare("SELECT * FROM CDS_chantier_lots WHERE chantier_id = ? ORDER BY ordre, cree_at");
    $s2->execute([$id]);
    $ch['lots'] = $s2->fetchAll(PDO::FETCH_ASSOC);

    // Intervenants
    $s3 = $db->prepare("SELECT * FROM CDS_chantier_intervenants WHERE chantier_id = ? ORDER BY role, nom");
    $s3->execute([$id]);
...
function createChantier($user) {
    ...
    $db->prepare("INSERT INTO CDS_chantiers (id, projet_id, nom, code, adresse, lat, lng,
    ...
    $db->prepare("INSERT INTO CDS_chantier_phases (id, chantier_id, nom, ordre, actif, avancement) VALUES (?,?,?,?,?,?)")
...
function updateChantier($id, $user) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantiers SET
...
function deleteChantier($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantiers WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listLots() {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_chantier_lots WHERE chantier_id=? ORDER BY ordre, cree_at");
    $stmt->execute([$cid]);
    $lots = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Attach phases for each lot
    $phStmt = $db->prepare("SELECT * FROM CDS_chantier_phases WHERE lot_id=? ORDER BY ordre ASC, nom ASC");
    foreach ($lots as &$lot) {
...
function createLot($user) {
    ...
    $dup = $db->prepare("SELECT id FROM CDS_chantier_lots WHERE chantier_id=? AND nom=?");
    $dup->execute([$b['chantier_id']??'', $b['nom']??'']);
    if ($dup->fetch()) { jsonError('Attention : lot déjà existant', 409); return; }

    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_chantier_lots (id, chantier_id, code, nom, entreprise, montant_marche, date_debut, date_fin_prevue, ordre, couleur)
                  VALUES (?,?,?,?,?,?,?,?,?,?)")
...
function updateLot($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_lots SET code=?, nom=?, entreprise=?, montant_marche=?,
...
function deleteLot($id) {
    ...
    $db->prepare("DELETE FROM CDS_chantier_phases WHERE lot_id=?")->execute([$id]);
    $db->prepare("DELETE FROM CDS_chantier_lots WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listJournal() {
    ...
    $dateTo = $_GET['date_to'] ?? '';
    $phase = $_GET['phase'] ?? '';
    $sql = "SELECT * FROM CDS_chantier_journal WHERE chantier_id=?";
    $params = [$cid];
    ...
    foreach ($rows as &$r) {
        $s2 = $db->prepare("SELECT * FROM CDS_chantier_effectifs WHERE journal_id=?");
        $s2->execute([$r['id']]);
...
function createJournal($user) {
    ...
    $num = 1;
    try {
        $s = $db->prepare("SELECT COALESCE(MAX(numero),0)+1 AS next_num FROM CDS_chantier_journal WHERE chantier_id=?");
        $s->execute([$cid]);
        $num = $s->fetch(PDO::FETCH_ASSOC)['next_num'] ?: 1;
    } catch (Exception $e) {
        // numero column may not exist yet in migration edge case
        $s2 = $db->prepare("SELECT COUNT(*)+1 AS next_num FROM CDS_chantier_journal WHERE chantier_id=?");
        $s2->execute([$cid]);
        $num = $s2->fetch(PDO::FETCH_ASSOC)['next_num'];
    }

    $db->prepare("INSERT INTO CDS_chantier_journal (id, chantier_id, numero, date_jour, heure_debut, heure_fin,
    ...
    if (!empty($b['effectifs']) && is_array($b['effectifs'])) {
        foreach ($b['effectifs'] as $eff) {
            ...
            $db->prepare("INSERT INTO CDS_chantier_effectifs (id, journal_id, chantier_id, entreprise, nb_ouvriers, nb_cadres, commentaire)
                          VALUES (?,?,?,?,?,?,?)")
...
function updateJournal($id, $user) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_journal SET date_jour=?, heure_debut=?, heure_fin=?,
                  phase_lot=?, meteo=?, temperature=?, effectif_total=?, activites=?, livraisons=?,
                  intervenants_presents=?, visiteurs=?, incidents_securite=?, retards=?, decisions=?,
    ...
    // Refresh effectifs
    $db->prepare("DELETE FROM CDS_chantier_effectifs WHERE journal_id=?")->execute([$id]);
    if (!empty($b['effectifs']) && is_array($b['effectifs'])) {
        $cid = $b['chantier_id'] ?? '';
        foreach ($b['effectifs'] as $eff) {
            $eid = bin2hex(random_bytes(16));
...
function deleteJournal($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_effectifs WHERE journal_id=?")->execute([$id]);
    $db->prepare("DELETE FROM CDS_chantier_journal WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}

function validerJournal($id, $user) {
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_journal SET valide_par=?, valide_at=NOW() WHERE id=?")
...
function listEffectifs() {
    $db = getDB();
    $cid = $_GET['chantier_id'] ?? '';
    $stmt = $db->prepare("SELECT e.*, j.date_jour FROM CDS_chantier_effectifs e
                          JOIN CDS_chantier_journal j ON j.id = e.journal_id
...
function saveEffectifs($user) {
    ...
    $jid = $b['journal_id'] ?? '';
    $cid = $b['chantier_id'] ?? '';
    $db->prepare("DELETE FROM CDS_chantier_effectifs WHERE journal_id=?")->execute([$jid]);
    if (!empty($b['effectifs']) && is_array($b['effectifs'])) {
        foreach ($b['effectifs'] as $eff) {
            $eid = bin2hex(random_bytes(16));
            $db->prepare("INSERT INTO CDS_chantier_effectifs (id, journal_id, chantier_id, entreprise, nb_ouvriers, nb_cadres, commentaire)
...
function listIntervenants() {
    $db = getDB();
    $cid = $_GET['chantier_id'] ?? '';
    $stmt = $db->prepare("SELECT * FROM CDS_chantier_intervenants WHERE chantier_id=? ORDER BY role, nom");
    $stmt->execute([$cid]);
    jsonOk($stmt->fetchAll(PDO::FETCH_ASSOC));
}
...
function createIntervenant($user) {
    ...
    $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_chantier_intervenants (id, chantier_id, role, nom, societe, tel, email, responsabilites, acces_portail)
                  VALUES (?,?,?,?,?,?,?,?,?)")
...
function updateIntervenant($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_intervenants SET role=?, nom=?, societe=?, tel=?, email=?, responsabilites=?, acces_portail=? WHERE id=?")
...
function deleteIntervenant($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_intervenants WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listTachesChantier() {
    ...
    $stmt = $db->prepare("SELECT t.*, l.nom AS lot_nom, l.couleur AS lot_couleur
                          FROM CDS_chantier_taches t
                          LEFT JOIN CDS_chantier_lots l ON l.id = t.lot_id
                          WHERE t.chantier_id=? ORDER BY t.ordre, t.date_debut");
    $stmt->execute([$cid]);
    $tasks = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Attach dependencies
    foreach ($tasks as &$t) {
        $s2 = $db->prepare("SELECT * FROM CDS_chantier_tache_deps WHERE task_id=?");
        $s2->execute([$t['id']]);
...
function createTacheChantier($user) {
    ...
    $db = getDB();
    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_chantier_taches (id, chantier_id, lot_id, parent_id, titre, date_debut, date_fin, duree_jours, avancement, est_jalon, est_critique, ordre)
                  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
    ...
    if (!empty($b['dependencies']) && is_array($b['dependencies'])) {
        foreach ($b['dependencies'] as $dep) {
            ...
            $db->prepare("INSERT INTO CDS_chantier_tache_deps (id, task_id, depends_on, type, lag_days) VALUES (?,?,?,?,?)")
...
function updateTacheChantier($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_taches SET lot_id=?, parent_id=?, titre=?, date_debut=?, date_fin=?,
    ...
    $db->prepare("DELETE FROM CDS_chantier_tache_deps WHERE task_id=?")->execute([$id]);
    if (!empty($b['dependencies']) && is_array($b['dependencies'])) {
        foreach ($b['dependencies'] as $dep) {
            $did = bin2hex(random_bytes(16));
            $db->prepare("INSERT INTO CDS_chantier_tache_deps (id, task_id, depends_on, type, lag_days) VALUES (?,?,?,?,?)")
...
function deleteTacheChantier($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_tache_deps WHERE task_id=?")->execute([$id]);
    $db->prepare("DELETE FROM CDS_chantier_taches WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listPhases() {
    $db = getDB();
    $rows = $db->query("SELECT * FROM CDS_chantier_phases ORDER BY ordre ASC, nom ASC")->fetchAll(PDO::FETCH_ASSOC);
    jsonOk($rows);
}
...
function createPhase($user) {
    ...
    $db = getDB();
    $id = bin2hex(random_bytes(16));
    $maxOrd = $db->query("SELECT COALESCE(MAX(ordre),0)+1 FROM CDS_chantier_phases")->fetchColumn();
    $db->prepare("INSERT INTO CDS_chantier_phases (id, nom, ordre, actif) VALUES (?,?,?,?)")
...
function updatePhase($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_phases SET nom=?, ordre=?, actif=? WHERE id=?")
...
function deletePhase($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_phases WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function exportJournalPDF($id) {
    ...
    $stmt = $db->prepare("SELECT j.*, c.nom AS chantier_nom, c.code AS chantier_code, c.adresse AS chantier_adresse,
                          ...
                          FROM CDS_chantier_journal j
                          LEFT JOIN CDS_chantiers c ON c.id = j.chantier_id
                          LEFT JOIN CDS_projets p ON p.id = c.projet_id
                          WHERE j.id=?");
    $stmt->execute([$id]);
    ...
    $agence = [];
    try {
        $sa = $db->query("SELECT cle, valeur FROM CDS_params WHERE cle LIKE 'agence_%'");
...
function getDashboard() {
    ...
    $ch = $db->prepare("SELECT * FROM CDS_chantiers WHERE id=?");
    $ch->execute([$cid]);
    $data = $ch->fetch(PDO::FETCH_ASSOC);
    if (!$data) jsonError('Chantier introuvable', 404);

    // Lots summary
    $s1 = $db->prepare("SELECT COUNT(*) as total, AVG(avancement) as avg_avancement FROM CDS_chantier_lots WHERE chantier_id=?");
    $s1->execute([$cid]);
    ...
    $data['reserves_stats'] = [];
    try {
        $s2 = $db->prepare("SELECT statut, COUNT(*) as nb FROM CDS_chantier_reserves WHERE chantier_id=? GROUP BY statut");
        $s2->execute([$cid]);
    ...
    $data['rfi_stats'] = [];
    try {
        $s3 = $db->prepare("SELECT statut, COUNT(*) as nb FROM CDS_chantier_rfi WHERE chantier_id=? GROUP BY statut");
        $s3->execute([$cid]);
    ...
    $data['visa_stats'] = [];
    try {
        $s4 = $db->prepare("SELECT statut, COUNT(*) as nb FROM CDS_chantier_visas WHERE chantier_id=? GROUP BY statut");
        $s4->execute([$cid]);
    ...
    try {
        $s5 = $db->prepare("SELECT type, COUNT(*) as nb FROM CDS_chantier_incidents WHERE chantier_id=? GROUP BY type");
        $s5->execute([$cid]);
    ...
    $data['recent_journal'] = [];
    try {
        $s6 = $db->prepare("SELECT * FROM CDS_chantier_journal WHERE chantier_id=? ORDER BY date_jour DESC LIMIT 5");
        $s6->execute([$cid]);
    ...
    try {
        $s7 = $db->prepare("SELECT COUNT(*) as nb FROM CDS_chantier_reunion_actions WHERE chantier_id=? AND statut != 'Clôturée'");
        $s7->execute([$cid]);
...
function listParamLots() {
    $db = getDB();
    $rows = $db->query("SELECT * FROM CDS_param_lots ORDER BY ordre ASC, nom ASC")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($rows as &$row) {
        $sp = $db->prepare("SELECT * FROM CDS_param_lot_phases WHERE param_lot_id=? ORDER BY ordre ASC, cree_at ASC");
        $sp->execute([$row['id']]);
...
function createParamLot($user) {
    ...
    $db = getDB();
    $id = bin2hex(random_bytes(16));
    $maxOrd = $db->query("SELECT COALESCE(MAX(ordre),0)+1 FROM CDS_param_lots")->fetchColumn();
    $db->prepare("INSERT INTO CDS_param_lots (id, nom, code, ordre, actif, couleur) VALUES (?,?,?,?,?,?)")
    ...
    $db->prepare("INSERT INTO CDS_param_lot_phases (id, param_lot_id, nom, ordre) VALUES (?,?,?,?)")
...
function updateParamLot($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_param_lots SET nom=?, code=?, ordre=?, actif=?, couleur=? WHERE id=?")
...
function deleteParamLot($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_param_lot_phases WHERE param_lot_id=?")->execute([$id]);
    $db->prepare("DELETE FROM CDS_param_lots WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listParamLotPhases() {
    ...
    $stmt = $db->prepare("SELECT * FROM CDS_param_lot_phases WHERE param_lot_id=? ORDER BY ordre ASC, cree_at ASC");
    $stmt->execute([$lotId]);
    jsonOk($stmt->fetchAll(PDO::FETCH_ASSOC));
}
...
function createParamLotPhase($user) {
    ...
    $db = getDB();
    $id = bin2hex(random_bytes(16));
    $maxOrd = $db->prepare("SELECT COALESCE(MAX(ordre),0)+1 FROM CDS_param_lot_phases WHERE param_lot_id=?");
    $maxOrd->execute([$b['param_lot_id']??'']);
    $ord = $maxOrd->fetchColumn();
    // Insert before "Phase de fin" (ordre 9999)
    if ($ord >= 9999) $ord = 9998;
    $db->prepare("INSERT INTO CDS_param_lot_phases (id, param_lot_id, nom, ordre, actif) VALUES (?,?,?,?,?)")
...
function updateParamLotPhase($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_param_lot_phases SET nom=?, ordre=?, actif=? WHERE id=?")
       ->execute([$b['nom']??'', $b['ordre']??0, $b['actif']??1, $id]);
    jsonOk(['updated' => true]);
}
...
function deleteParamLotPhase($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_param_lot_phases WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listLotPhases() {
    $db = getDB();
    $lotId = $_GET['lot_id'] ?? '';
    $stmt = $db->prepare("SELECT * FROM CDS_chantier_lot_phases WHERE lot_id=? ORDER BY ordre ASC, cree_at ASC");
    $stmt->execute([$lotId]);
    jsonOk($stmt->fetchAll(PDO::FETCH_ASSOC));
}
...
function createLotPhase($user) {
    ...
    $dup = $db->prepare("SELECT id FROM CDS_chantier_lot_phases WHERE lot_id=? AND nom=?");
    $dup->execute([$b['lot_id']??'', $b['nom']??'']);
    if ($dup->fetch()) { jsonError('Attention : phase déjà existante dans ce lot', 409); return; }

    $id = bin2hex(random_bytes(16));
    $db->prepare("INSERT INTO CDS_chantier_lot_phases (id, chantier_id, lot_id, nom, avancement, ordre) VALUES (?,?,?,?,?,?)")
...
function updateLotPhase($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CDS_chantier_lot_phases SET nom=?, avancement=?, ordre=? WHERE id=?")
       ->execute([$b['nom']??'', $b['avancement']??0, $b['ordre']??0, $id]);
    // Recalculate lot avancement = average of its phases
    $lotId = $b['lot_id'] ?? '';
    if ($lotId) {
        $avg = $db->prepare("SELECT ROUND(AVG(avancement)) FROM CDS_chantier_lot_phases WHERE lot_id=?");
        $avg->execute([$lotId]);
        $newAvg = (int)$avg->fetchColumn();
        $db->prepare("UPDATE CDS_chantier_lots SET avancement=? WHERE id=?")->execute([$newAvg, $lotId]);
    }
    jsonOk(['updated' => true]);
}
...
function deleteLotPhase($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CDS_chantier_lot_phases WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function addAllLotsToChantier($user) {
    ...
    $ch = $db->prepare("SELECT date_debut, date_fin_prevue, lot_depart, lot_fin FROM CDS_chantiers WHERE id=?");
    $ch->execute([$chantierId]);
    $chData = $ch->fetch(PDO::FETCH_ASSOC);

    // Get existing lot names in this chantier (to skip duplicates)
    $existStmt = $db->prepare("SELECT nom FROM CDS_chantier_lots WHERE chantier_id=?");
    ...
    $stmt = $db->query("SELECT * FROM CDS_param_lots WHERE actif=1 ORDER BY ordre ASC");
    ...
    foreach ($paramLots as $pl) {
        ...
        $sp = $db->prepare("SELECT * FROM CDS_param_lot_phases WHERE param_lot_id=? AND actif=1 ORDER BY ordre ASC");
        $sp->execute([$pl['id']]);
