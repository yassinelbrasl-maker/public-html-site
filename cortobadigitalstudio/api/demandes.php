...
function getAll() {
    ...
    $stmt = $db->prepare('SELECT * FROM CDS_demandes WHERE ' . implode(' AND ', $where) . ' ORDER BY cree_at DESC');
    $stmt->execute($params);
...
function getOne(string $id) {
    $db   = getDB();
    $stmt = $db->prepare('SELECT * FROM CDS_demandes WHERE id = ?');
    $stmt->execute([$id]);
    $d = $stmt->fetch();
...
function createPublic() {
    ...
    $db->prepare('
        INSERT INTO CDS_demandes (id, nom_projet, prenom, nom, tel, whatsapp, email,
    ...
    // Répondre au client immédiatement (ne pas bloquer sur l'envoi email)
    // On envoie la réponse JSON, puis on ferme la connexion, puis on envoie l'email
    $responseData = json_encode(['ok' => true, 'data' => ['id' => $id, 'statut' => 'nouvelle']]);
    ...
    // ── Envoi email de notification à l'admin (après réponse) ──
    try {
        $stmtEmail = $db->prepare("SELECT setting_value FROM CDS_settings WHERE setting_key = ?");
        ...
        $emailCli  = htmlspecialchars($body['email'] ?? '-');
        $surfEst   = !empty($body['surface_estimee']) ? number_format(floatval($body['surface_estimee']), 0, ',', ' ') . ' m2' : '-';
        ...
        $subject = '=?UTF-8?B?' . base64_encode('Nouvelle demande configurateur - ' . $nomProjet) . '?=';
        $htmlMsg = '<!DOCTYPE html><html><head><meta charset="utf-8"></head>'
            ...
            . '<tr><td style="padding:8px 12px;border:1px solid #333;color:#999;font-size:13px">Telephone</td>'
            ...
            . '<td style="padding:8px 12px;border:1px solid #333;font-size:13px;color:#e8e4dc">' . $emailCli . '</td></tr>'
            ...
            . '<tr><td style="padding:8px 12px;border:1px solid #333;color:#999;font-size:13px">Operation</td>'
            ...
            . '<tr><td style="padding:8px 12px;border:1px solid #333;color:#999;font-size:13px">Surface estimee</td>'
            . '<td style="padding:8px 12px;border:1px solid #333;font-size:13px;color:#e8e4dc">' . $surfEst . '</td></tr>'
            . '<tr><td style="padding:8px 12px;border:1px solid #333;color:#999;font-size:13px">Cout estime</td>'
            . '<td style="padding:8px 12px;border:1px solid #333;font-size:13px;font-weight:bold;color:#c8a96e">' . $coutLow . ' - ' . $coutHigh . '</td></tr>'
            . '</table>'
            . '<p style="color:#888;font-size:12px">Connectez-vous a la plateforme pour traiter cette demande.</p>'
            . '</td></tr>'
...
function handlePut($id, array $user) {
    ...
    switch ($action) {
        case 'update_statut':
            ...
            $db->prepare('UPDATE CDS_demandes SET statut=?, traite_par=?, traite_at=NOW() WHERE id=?')
        ...
        case 'convertir_client':
            ...
            try {
                $db->prepare('
                    INSERT INTO CDS_clients (id, code, num_client, type, prenom, nom, raison, matricule,
            ...
            $db->prepare('UPDATE CDS_demandes SET client_id=?, statut=?, traite_par=?, traite_at=NOW() WHERE id=?')
        ...
        case 'convertir_projet':
            ...
            $stmtCli = $db->prepare('SELECT display_nom, code FROM CDS_clients WHERE id = ?');
            ...
            $db->prepare('
                INSERT INTO CDS_projets (id, code, nom, client, client_code, annee, phase, statut, type_bat,
            ...
            if ($client && !empty($client['code'])) {
                $db->prepare('UPDATE CDS_clients SET projets = projets + 1 WHERE code = ?')->execute([$client['code']]);
            }

            $db->prepare('UPDATE CDS_demandes SET projet_id=?, statut=?, traite_par=?, traite_at=NOW() WHERE id=?')
        ...
        case 'creer_devis':
            ...
            if ($devisId) {
                $db->prepare('UPDATE CDS_demandes SET devis_id=?, statut=?, traite_par=?, traite_at=NOW() WHERE id=?')
        ...
        case 'accepter_demande':
            ...
            try {
                ...
                $stmtCli2 = $db->prepare('SELECT display_nom FROM CDS_clients WHERE id = ?');
                ...
                $devisCount = (int)$db->query('SELECT COUNT(*) FROM CDS_devis')->fetchColumn();
                ...
                $db->prepare('
                    INSERT INTO CDS_devis (id, numero, client, client_id, projet_id, montant_ht, tva, montant_ttc,
                ...
                $db->prepare('UPDATE CDS_demandes SET client_id=?, projet_id=?, devis_id=?, statut=?, traite_par=?, traite_at=NOW() WHERE id=?')
                ...
                if ($mopResult && $coutConstruction > 0) {
                    $stmtH = $db->prepare("INSERT INTO CDS_projets_honoraires (projet_id, mission_phase, mission_label, mode, taux_pct, montant_prevu, ordre) VALUES (?,?,?,?,?,?,?)");
                    foreach ($mopResult['phases'] as $p) {
                        $stmtH->execute([$projetId, $p['phase'], $p['label'], 'mop', $p['pct'], $p['montant'], $p['ordre']]);
                    }
                    $db->prepare("UPDATE CDS_projets SET honoraires_mode = 'mop', honoraires = ?, honoraires_prevus = ?, cout_construction = ? WHERE id = ?")
  ...
  <tr><td style="padding:16px 30px;background:#f8f8f8;border-top:1px solid #eee">
    <p style="color:#999;font-size:11px;margin:0">CORTOBA Atelier d\'Architecture · Tunis, Tunisie<br>cortobaarchitecture@gmail.com</p>
        ...
        case 'update_remarques':
            ...
            $db->prepare('UPDATE CDS_demandes SET remarques=?, traite_par=?, traite_at=NOW() WHERE id=?')
        ...
        default:
            ...
            $db->prepare('
                UPDATE CDS_demandes SET statut=?, remarques=?, traite_par=?, traite_at=NOW() WHERE id=?
            ')->execute([
...
function remove($id, array $user) {
    ...
    $stmt = $db->prepare('SELECT nom_projet, prenom, nom FROM CDS_demandes WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    if (!$row) jsonError('Demande introuvable', 404);
    $label = trim(($row['nom_projet'] ?? '') . ' — ' . ($row['prenom'] ?? '') . ' ' . ($row['nom'] ?? '')) ?: 'Demande';
    if (!moveToCorbeille($db, 'CDS_demandes', $id, $label, $user['name'] ?? 'unknown')) {
