...
//  Usage : /cortobadigitalstudio/tools/seed_archives.php?key=CAS_SEED_2026
...
$projects = [
    ...
    ['num'=>'17','year'=>2025,'code3'=>'BCA','folder'=>'17_25_BCDS_BOUCHAMIA AMANI',              'type'=>'physique','nom'=>'BOUCHAMIA',       'prenom'=>'AMANI'],
...
$stmt = $db->query('SELECT COALESCE(MAX(num_client), 0) AS mx FROM CDS_clients');
...
foreach ($projects as $p) {
    ...
    $stmt = $db->prepare('SELECT id, code FROM CDS_clients WHERE display_nom = ? LIMIT 1');
    ...
    if ($existing) {
        ...
        $db->prepare('UPDATE CDS_clients SET projets = projets + 0 WHERE id = ?')->execute([$existing['id']]);
        continue;
    }
...
foreach ($projects as $p) {
    ...
    $stmt = $db->prepare('SELECT id FROM CDS_projets WHERE code = ? LIMIT 1');
    ...
    try {
        ...
        if ($client) {
            $db->prepare('UPDATE CDS_clients SET projets = projets + 1 WHERE id = ?')
               ->execute([$client['id']]);
        }
