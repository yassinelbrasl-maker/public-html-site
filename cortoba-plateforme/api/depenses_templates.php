...
function ensureTemplatesTable() {
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `CA_depenses_templates` (
...
try {
    ...
    if ($method === 'GET') {
        if ($action === 'due') {
            ...
            $sql = "SELECT * FROM CA_depenses_templates
                    WHERE status = 'active'
        ...
        if ($id) {
            $stmt = $db->prepare("SELECT * FROM CA_depenses_templates WHERE id = ?");
        ...
        $rows = $db->query("SELECT * FROM CA_depenses_templates WHERE status != 'cancelled' ORDER BY next_due_date ASC")
    ...
    // ── POST : créer OU action=advance ──
        ...
        if ($action === 'advance') {
            ...
            $stmt = $db->prepare("SELECT next_due_date, frequency, end_date FROM CA_depenses_templates WHERE id = ?");
            $stmt->execute(array($id));
            ...
            if (!empty($t['end_date']) && $nextDue > $t['end_date']) {
                $db->prepare("UPDATE CA_depenses_templates SET status = 'cancelled' WHERE id = ?")->execute(array($id));
                jsonOk(array('id' => $id, 'status' => 'cancelled', 'next_due_date' => $nextDue));
            }
            $db->prepare("UPDATE CA_depenses_templates SET next_due_date = ? WHERE id = ?")
        ...
        $sql  = "INSERT INTO CA_depenses_templates (`" . implode('`,`', $keys) . "`) VALUES (" . implode(',', $ph) . ")";
    ...
    // ── PUT : mise à jour ──
        ...
        $vals[] = $id;
        $db->prepare("UPDATE CA_depenses_templates SET " . implode(', ', $set) . " WHERE id = ?")->execute($vals);
