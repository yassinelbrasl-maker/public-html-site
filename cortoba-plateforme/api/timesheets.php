...
function ensureTimesheetsTable() {
    $db = getDB();
    $db->exec("CREATE TABLE IF NOT EXISTS `CA_timesheets` (
...
try {
    ...
    if ($method === 'GET') {
        if ($id) {
            $stmt = $db->prepare('SELECT * FROM CA_timesheets WHERE id = ?');
            $stmt->execute([$id]);
        ...
        $sql = 'SELECT t.*, p.nom AS projet_nom, p.code AS projet_code, tk.titre AS tache_titre
                FROM CA_timesheets t
                LEFT JOIN CA_projets p ON p.id COLLATE utf8mb4_unicode_ci = t.projet_id COLLATE utf8mb4_unicode_ci
                LEFT JOIN CA_taches tk ON tk.id COLLATE utf8mb4_unicode_ci = t.tache_id COLLATE utf8mb4_unicode_ci
    ...
    if ($method === 'POST') {
        ...
        $db->prepare('INSERT INTO CA_timesheets (id,user_id,user_name,projet_id,tache_id,date_jour,hours_spent,is_billable,commentaire) VALUES (?,?,?,?,?,?,?,?,?)')
           ->execute([
            $newId,
        ...
        if (!empty($body['tache_id'])) {
            try {
                $stmtH = $db->prepare('SELECT COALESCE(SUM(hours_spent),0) AS h FROM CA_timesheets WHERE tache_id = ?');
                $stmtH->execute([$body['tache_id']]);
                $hTotal = floatval($stmtH->fetch()['h']);
                $db->prepare('UPDATE CA_taches SET heures_reelles = ? WHERE id = ?')->execute([$hTotal, $body['tache_id']]);
    ...
    if ($method === 'PUT') {
        ...
        $vals[] = $id;
        $db->prepare('UPDATE CA_timesheets SET ' . implode(',', $fields) . ' WHERE id = ?')->execute($vals);
        jsonOk(['updated' => $id]);
        return;
    }
    ...
    if ($method === 'DELETE') {
        ...
        $stmt = $db->prepare('SELECT tache_id FROM CA_timesheets WHERE id = ?');
        $stmt->execute([$id]);
        $row = $stmt->fetch();
        $db->prepare('DELETE FROM CA_timesheets WHERE id = ?')->execute([$id]);
        if ($row && $row['tache_id']) {
            try {
                $stmtH = $db->prepare('SELECT COALESCE(SUM(hours_spent),0) AS h FROM CA_timesheets WHERE tache_id = ?');
                $stmtH->execute([$row['tache_id']]);
                $hTotal = floatval($stmtH->fetch()['h']);
                $db->prepare('UPDATE CA_taches SET heures_reelles = ? WHERE id = ?')->execute([$hTotal, $row['tache_id']]);
            } catch (\Throwable $e) {}
        }
        jsonOk(['deleted' => $id]);
        return;
