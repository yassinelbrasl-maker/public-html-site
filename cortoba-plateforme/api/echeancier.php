...
function ensureEcheancierSchema() {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CA_echeancier` (
...
function saveEcheance($user) {
    ...
    if ($id) {
        // Update
        $db->prepare("UPDATE CA_echeancier SET projet_id = ?, mission_phase = ?, description = ?, montant_prevu = ?, date_prevue = ?, statut = ? WHERE id = ?")
           ->execute([
               $body['projet_id'] ?? '',
        ...
        // Insert
        $db->prepare("INSERT INTO CA_echeancier (projet_id, mission_phase, description, montant_prevu, date_prevue, statut, cree_par) VALUES (?,?,?,?,?,?,?)")
           ->execute([
               $body['projet_id'] ?? '',
               $body['mission_phase'] ?? null,
               $body['description'] ?? null,
    ...
    $s = $db->prepare("SELECT * FROM CA_echeancier WHERE id = ?");
    $s->execute([$id]);
...
function deleteEcheance($user) {
    ...
    $db = getDB();
    $db->prepare("DELETE FROM CA_echeancier WHERE id = ?")->execute([$id]);
    jsonOk(['deleted' => $id]);
}
...
function generateFromHonoraires($user) {
    ...
    $db->prepare("DELETE FROM CA_echeancier WHERE projet_id = ? AND statut = 'prevu'")->execute([$projetId]);

    $startDate = new \DateTime($body['date_debut'] ?? 'now');
    $intervalMonths = (int)($body['intervalle_mois'] ?? 2);

    $stmt = $db->prepare("INSERT INTO CA_echeancier (projet_id, mission_phase, description, montant_prevu, date_prevue, cree_par) VALUES (?,?,?,?,?,?)");
...
function linkFacture($user) {
    ...
    $db = getDB();
    $db->prepare("UPDATE CA_echeancier SET facture_id = ?, statut = 'facture' WHERE id = ?")
