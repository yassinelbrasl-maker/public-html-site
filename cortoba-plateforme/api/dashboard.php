...
function getAll() {
    ...
    $hasDateEmission    = colExists($db, 'CA_factures', 'date_emission');
    $hasNetPayer        = colExists($db, 'CA_factures', 'net_payer');
    $hasMontantPaye     = colExists($db, 'CA_factures', 'montant_paye');
    $hasDepMontantTtc   = colExists($db, 'CA_depenses', 'montant_ttc');
    $hasUsersHeuresMois = colExists($db, 'cortoba_users', 'heures_mois');
    $hasPaimClientsTbl  = tableExists($db, 'CA_paiements_clients');
    $hasHonoPrevus      = colExists($db, 'CA_projets', 'honoraires_prevus');
    $hasProgression     = colExists($db, 'CA_taches', 'progression');
    ...
    // ── Source de vérité du CA : paiements clients encaissés (table CA_paiements_clients)
    //    Fallback si vide : factures au statut 'Payée'.
    $useCaPaim = false;
    if ($hasPaimClientsTbl) {
        try {
            $cntP = (int)$db->query("SELECT COUNT(*) FROM CA_paiements_clients")->fetchColumn();
            $useCaPaim = $cntP > 0;
    ...
    try {
        if ($useCaPaim) {
            $stmt = $db->prepare("SELECT COALESCE(SUM(montant),0) FROM CA_paiements_clients
            ...
            $stmt = $db->prepare("SELECT COALESCE(SUM(montant_ttc),0) FROM CA_factures
    ...
    try {
        ...
        $stmt = $db->query("SELECT phase, COUNT(*) AS nb FROM CA_projets
    ...
    // ── Dépenses du mois ──
    try {
        $stmt = $db->prepare("SELECT COALESCE(SUM($depMontantExpr),0) FROM CA_depenses WHERE date_dep >= ?");
        ...
        $stmt = $db->prepare("SELECT COALESCE(SUM($depMontantExpr),0) FROM CA_depenses
    ...
    try {
        if ($hasUsersHeuresMois) {
            $stmt = $db->query("SELECT COUNT(*) AS nb, COALESCE(SUM(heures_mois),0) AS tot
                FROM cortoba_users WHERE statut = 'Actif'");
            $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    try {
        $stmt = $db->prepare("SELECT COALESCE(SUM(hours_spent),0) FROM CA_timesheets
            WHERE date_jour >= ?");
        $stmt->execute([$monthStart]);
    ...
    try {
        if ($useCaPaim) {
            $stmt = $db->prepare("SELECT MONTH(date_paiement) AS mois, SUM(montant) AS total
                FROM CA_paiements_clients WHERE YEAR(date_paiement) = ?
            ...
            $stmt = $db->prepare("SELECT MONTH($factDateCol) AS mois, SUM(montant_ttc) AS total
                FROM CA_factures WHERE statut = 'Payée' AND YEAR($factDateCol) = ?
    ...
    try {
        $progCol = $hasProgression
            ? "(SELECT AVG(progression) FROM CA_taches WHERE projet_id COLLATE utf8mb4_unicode_ci = p.id COLLATE utf8mb4_unicode_ci)"
            : "NULL";
        $stmt = $db->query("SELECT p.id, p.nom, p.phase, p.statut,
            (SELECT COUNT(*) FROM CA_taches
        ...
        try {
            $stmt = $db->query("SELECT id, nom, phase, statut FROM CA_projets
                WHERE statut IN ('En cours','Actif')
    ...
    // Derniers paiements (priorise CA_paiements_clients)
    try {
        if ($useCaPaim) {
            $stmt = $db->query("SELECT pc.montant, pc.date_paiement AS dt, pc.cree_at,
                ...
                LEFT JOIN CA_clients cl ON cl.id COLLATE utf8mb4_unicode_ci = pc.client_id  COLLATE utf8mb4_unicode_ci
    ...
    try {
        $stmt = $db->prepare("SELECT COALESCE(NULLIF(categorie,''),'Autre') AS categorie,
            ...
            FROM CA_depenses WHERE date_dep >= ?
    ...
    if ($hasHonoPrevus) {
        try {
            ...
            $stmt = $db->query("SELECT
                ...
                FROM CA_projets WHERE statut IN ('En cours','Actif')");
...
function getDiag() {
    ...
    try {
        $stmt = $db->query("SELECT COUNT(*) AS total,
            GROUP_CONCAT(DISTINCT statut) AS statuts FROM CA_projets");
        $diag['projets'] = $stmt->fetch(\PDO::FETCH_ASSOC);
    } catch (\Throwable $e) { $diag['projets'] = 'ERR: ' . $e->getMessage(); }

    try {
        $stmt = $db->query("SELECT COUNT(*) AS total,
            GROUP_CONCAT(DISTINCT statut) AS statuts FROM CA_devis");
        $diag['devis'] = $stmt->fetch(\PDO::FETCH_ASSOC);
    } catch (\Throwable $e) { $diag['devis'] = 'ERR: ' . $e->getMessage(); }

    try {
        $stmt = $db->query("SELECT COUNT(*) AS total,
            GROUP_CONCAT(DISTINCT statut) AS statuts FROM CA_factures");
        $diag['factures'] = $stmt->fetch(\PDO::FETCH_ASSOC);

        // Détection colonnes facultatives
        $diag['factures_cols'] = [
            'date_emission' => colExists($db, 'CA_factures', 'date_emission'),
            'net_payer'     => colExists($db, 'CA_factures', 'net_payer'),
            'montant_paye'  => colExists($db, 'CA_factures', 'montant_paye'),
        ];

        $colDate = colExists($db, 'CA_factures', 'date_emission')
            ? 'date_emission, date_facture, date_paiement'
            : 'date_facture, date_paiement';
        $stmt = $db->query("SELECT id, statut, $colDate, montant_ttc FROM CA_factures LIMIT 3");
    ...
    try {
        $stmt = $db->query("SELECT COUNT(*) AS total,
            GROUP_CONCAT(DISTINCT statut) AS statuts FROM CA_taches");
    ...
    try {
        $stmt = $db->query("SELECT COUNT(*) AS total,
            COALESCE(SUM(montant),0) AS somme_montant FROM CA_paiements_clients");
