...
function listIncidents() {
    ...
    $type = $_GET['type'] ?? '';
    $statut = $_GET['statut'] ?? '';
    $sql = "SELECT * FROM CA_chantier_incidents WHERE chantier_id=?";
    $params = [$cid];
...
function getIncident($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM CA_chantier_incidents WHERE id=?");
    $stmt->execute([$id]);
...
function createIncident($user) {
    ...
    $db->prepare("INSERT INTO CA_chantier_incidents (id, chantier_id, type, gravite, titre, description,
...
function updateIncident($id) {
    $b = getBody();
    $db = getDB();
    $db->prepare("UPDATE CA_chantier_incidents SET type=?, gravite=?, titre=?, description=?,
                  zone=?, entreprise=?, personnes_impliquees=?, date_incident=?, mesures_immediates=?,
...
function deleteIncident($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CA_chantier_incidents WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
...
function listInspections() {
    $db = getDB();
    $cid = $_GET['chantier_id'] ?? '';
    $stmt = $db->prepare("SELECT * FROM CA_chantier_inspections WHERE chantier_id=? ORDER BY date_inspection DESC");
    $stmt->execute([$cid]);
...
function getInspection($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM CA_chantier_inspections WHERE id=?");
    $stmt->execute([$id]);
    $r = $stmt->fetch(PDO::FETCH_ASSOC);
...
function createInspection($user) {
    ...
    $db->prepare("INSERT INTO CA_chantier_inspections (id, chantier_id, titre, date_inspection,
...
function updateInspection($id) {
    ...
    $db->prepare("UPDATE CA_chantier_inspections SET titre=?, date_inspection=?, inspecteur=?,
...
function deleteInspection($id) {
    $db = getDB();
    $db->prepare("DELETE FROM CA_chantier_inspections WHERE id=?")->execute([$id]);
    jsonOk(['deleted' => true]);
}
