...
function ensureHonorairesSchema() {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CA_honoraires_grille` (
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS `CA_projets_honoraires` (
      `id` int unsigned NOT NULL AUTO_INCREMENT,
    ...
    // ALTER CA_projets — add fee tracking columns
    $alters = [
    ...
    foreach ($alters as $a) {
        try { $db->exec("ALTER TABLE CA_projets $a"); } catch (\Throwable $e) {}
    }

    // ALTER CA_factures — add mission_phase, montant_paye
    $fa = [
    ...
    $count = (int)$db->query("SELECT COUNT(*) FROM CA_honoraires_grille")->fetchColumn();
    if ($count === 0) {
        $db->exec("INSERT INTO CA_honoraires_grille (tranche_min, tranche_max, taux_global) VALUES
            (0, 100000, 8.000),
            (100000, 500000, 6.000),
...
function calculMOP($coutConstruction) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM CA_honoraires_grille WHERE actif = 1 AND tranche_min <= ? AND (tranche_max IS NULL OR tranche_max > ?) ORDER BY tranche_min ASC LIMIT 1");
...
function refreshProjetHonoraires($projetId) {
    ...
    $stmt = $db->prepare("SELECT * FROM CA_projets_honoraires WHERE projet_id = ? ORDER BY ordre ASC");
    ...
    foreach ($phases as $ph) {
        ...
        $s = $db->prepare("SELECT COALESCE(SUM(COALESCE(net_payer, montant_ttc, 0)),0) FROM CA_factures WHERE projet_id = ? AND mission_phase = ? AND statut != 'Annulée'");
        ...
        $db->prepare("UPDATE CA_projets_honoraires SET montant_facture = ?, montant_encaisse = ?, modifie_at = NOW() WHERE projet_id = ? AND mission_phase = ?")
    ...
    if ($totalPrevu > 0) {
        ...
        $s = $db->prepare("SELECT valeur FROM CA_parametres WHERE cle = 'alert_thresholds'");
        $s->execute();
    ...
    $db->prepare("UPDATE CA_projets SET honoraires_prevus = ?, honoraires_engages = ?, honoraires_factures = ?, honoraires_encaisses = ?, alerte_budget = ? WHERE id = ?")
...
function getGrille() {
    $db = getDB();
    $rows = $db->query("SELECT * FROM CA_honoraires_grille WHERE actif = 1 ORDER BY tranche_min ASC")->fetchAll(\PDO::FETCH_ASSOC);
    $phases = getMOPPhases();
...
function saveGrille($user) {
    ...
    try {
        // Deactivate existing
        $db->exec("UPDATE CA_honoraires_grille SET actif = 0");

        $stmt = $db->prepare("INSERT INTO CA_honoraires_grille (tranche_min, tranche_max, taux_global, esquisse_pct, aps_pct, apd_pct, pro_dce_pct, suivi_det_pct, aor_pct, actif) VALUES (?,?,?,?,?,?,?,?,?,1)");
        foreach ($rows as $r) {
            $stmt->execute([
                $r['tranche_min'] ?? 0,
                $r['tranche_max'] ?? null,
...
function getProjetHonoraires() {
    ...
    // Project info
    $s = $db->prepare("SELECT id, code, nom, cout_construction, honoraires, honoraires_mode, honoraires_prevus, honoraires_engages, honoraires_factures, honoraires_encaisses, alerte_budget FROM CA_projets WHERE id = ?");
    $s->execute([$projetId]);
    $projet = $s->fetch(\PDO::FETCH_ASSOC);
    if (!$projet) jsonError('Projet introuvable', 404);

    // Phases
    $s = $db->prepare("SELECT * FROM CA_projets_honoraires WHERE projet_id = ? ORDER BY ordre ASC");
    $s->execute([$projetId]);
...
function initProjetHonoraires($user) {
    ...
    // Get project
    $s = $db->prepare("SELECT id, cout_construction, honoraires FROM CA_projets WHERE id = ?");
    $s->execute([$projetId]);
    $projet = $s->fetch(\PDO::FETCH_ASSOC);
    if (!$projet) jsonError('Projet introuvable', 404);

    // Delete existing
    $db->prepare("DELETE FROM CA_projets_honoraires WHERE projet_id = ?")->execute([$projetId]);
    ...
    if ($mode === 'mop') {
        ...
        $stmt = $db->prepare("INSERT INTO CA_projets_honoraires (projet_id, mission_phase, mission_label, mode, taux_pct, montant_prevu, ordre) VALUES (?,?,?,?,?,?,?)");
        foreach ($calcul['phases'] as $p) {
            $stmt->execute([$projetId, $p['phase'], $p['label'], 'mop', $p['pct'], $p['montant'], $p['ordre']]);
        }

        // Update project
        $db->prepare("UPDATE CA_projets SET honoraires_mode = 'mop', honoraires = ?, honoraires_prevus = ? WHERE id = ?")
        ...
        $totalPrevu = 0;
        $stmt = $db->prepare("INSERT INTO CA_projets_honoraires (projet_id, mission_phase, mission_label, mode, taux_pct, montant_prevu, ordre) VALUES (?,?,?,'custom',?,?,?)");
        ...
        $db->prepare("UPDATE CA_projets SET honoraires_mode = 'custom', honoraires = ?, honoraires_prevus = ? WHERE id = ?")
...
function updateProjetPhase($user) {
    ...
    $vals[] = $id;
    $db->prepare("UPDATE CA_projets_honoraires SET " . implode(',', $sets) . " WHERE id = ?")->execute($vals);

    // Get projet_id to refresh
    $s = $db->prepare("SELECT projet_id FROM CA_projets_honoraires WHERE id = ?");
    $s->execute([$id]);
    $projetId = $s->fetchColumn();
    if ($projetId) refreshProjetHonoraires($projetId);

    $s = $db->prepare("SELECT * FROM CA_projets_honoraires WHERE id = ?");
    $s->execute([$id]);
...
function doRefreshProjet() {
    ...
    $db = getDB();
    $s = $db->prepare("SELECT honoraires_prevus, honoraires_engages, honoraires_factures, honoraires_encaisses, alerte_budget FROM CA_projets WHERE id = ?");
    $s->execute([$projetId]);
    jsonOk($s->fetch(\PDO::FETCH_ASSOC));
}
...
function getValeurAcquise() {
    ...
    $s = $db->prepare("SELECT mission_phase, mission_label, montant_prevu, montant_facture, montant_encaisse, progression_prevue, progression_reelle, ordre FROM CA_projets_honoraires WHERE projet_id = ? ORDER BY ordre ASC");
    $s->execute([$projetId]);
    $phases = $s->fetchAll(\PDO::FETCH_ASSOC);
...
function getDashboardHonoraires() {
    ...
    $stmt = $db->query("SELECT id, code, nom, honoraires_prevus, honoraires_engages, honoraires_factures, honoraires_encaisses, alerte_budget FROM CA_projets WHERE statut IN ('En cours','Actif') AND honoraires_prevus > 0 ORDER BY alerte_budget DESC, honoraires_prevus DESC");
