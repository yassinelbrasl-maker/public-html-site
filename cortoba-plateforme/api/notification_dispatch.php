...
if (!function_exists('dispatchNotification')) {
    ...
    function _getNotifPrefs(PDO $db, string $userId, string $type): array {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT * FROM CA_notification_prefs WHERE user_id = ? AND notif_type = ?");
    ...
    function _sendNotifEmail(PDO $db, string $userId, string $type, string $title,
        ...
        // Construire le lien
        $baseUrl = 'https://cortobaarchitecture.com/cortoba-plateforme/plateforme-nas.html';
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:20px auto;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08)">
  ...
  <tr><td style="padding:16px 30px;background:#f8f8f8;border-top:1px solid #eee">
    ...
    function _sendNotifPush(PDO $db, string $userId, string $type, string $title,
        ...
        $stmt = $db->prepare("SELECT * FROM CA_push_subscriptions WHERE user_id = ?");
        $stmt->execute([$userId]);
        $subs = $stmt->fetchAll();
        ...
        $payload = json_encode([
            'title'   => $title,
            'body'    => $message ?? '',
            'icon'    => '/cortoba-plateforme/img/cortoba-icon-192.png',
            'badge'   => '/cortoba-plateforme/img/cortoba-badge-72.png',
            'tag'     => $type . '_' . ($linkId ?? bin2hex(random_bytes(4))),
            'data'    => [
                'type'     => $type,
                'linkPage' => $linkPage,
                'linkId'   => $linkId,
                'url'      => '/cortoba-plateforme/plateforme-nas.html' . ($linkPage ? '#page=' . $linkPage : ''),
            ],
        ...
        foreach ($subs as $sub) {
            try {
                ...
                if (strpos($e->getMessage(), '410') !== false || strpos($e->getMessage(), '404') !== false) {
                    $db->prepare("DELETE FROM CA_push_subscriptions WHERE id = ?")
                       ->execute([$sub['id']]);
                }
            }
        }
