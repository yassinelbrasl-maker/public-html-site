...
try {
    if ($action === 'demander') {
        ...
        // Trouver tous les admins (CA_accounts) + gérants (cortoba_users avec is_admin=1)
        $admins = array();

        $stmt = $db->query("SELECT id, name FROM CA_accounts");
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $a) {
            $admins[] = $a;
        }

        try {
            $stmt2 = $db->query("SELECT id, CONCAT(prenom, ' ', nom) AS name FROM cortoba_users WHERE is_admin = 1 AND statut != 'Inactif'");
        ...
        $stmt = $db->prepare("SELECT id, cree_par, description, montant_ttc FROM CA_depenses WHERE id = ?");
        ...
        // Chercher dans cortoba_users
        try {
            $stmt2 = $db->prepare("SELECT id FROM cortoba_users WHERE CONCAT(prenom, ' ', nom) = ? LIMIT 1");
            $stmt2->execute(array($targetName));
            $member = $stmt2->fetch(PDO::FETCH_ASSOC);
            if ($member) $targetUserId = $member['id'];
        } catch (\Throwable $e) { }

        // Sinon dans CA_accounts
        if (!$targetUserId) {
            $stmt3 = $db->prepare("SELECT id FROM CA_accounts WHERE name = ? LIMIT 1");
